...
- **Public repo** — `chris-yyau/busdriver` is public; you are the sole admin (only approval-capable human — forkers who open PRs are not). Merges are gated by required status checks + sole-admin access. **Solo-admin auto-escalation is enabled** (`.claude/pr-grind-auto-admin-solo.local`, gitignored) but **currently inert**: `main` has no required-review rule (deliberate solo-repo choice — `required_pull_request_reviews: null` since the helmet rollout), so there is no approver gap for the opt-in to bridge. The real merge gate is the 6 required status checks with `strict` on (the `mergeStateStatus=BEHIND` enforcer) plus `enforce_admins=false`; an admin squash-merges (`--squash --delete-branch`), adding `--admin` only to clear a `BEHIND` head. The opt-in self-revokes if a second approval-capable human is ever added. Local tooling still preferred over remote CI for development workflow.
