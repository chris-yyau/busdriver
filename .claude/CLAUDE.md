...
- **Public repo** — `chris-yyau/busdriver` is public; you are the sole admin (only approval-capable human — forkers who open PRs are not). Merges are gated by required status checks + sole-admin access. **Solo-admin auto-escalation is enabled** (`.claude/pr-grind-auto-admin-solo.local`, gitignored): `main` classic branch protection has `required_approving_review_count=1` and `strict` status checks (the `mergeStateStatus=BEHIND` enforcer), but `enforce_admins=false` — so the sole admin is exempt from both, and merges clean PRs with `gh pr merge --squash --delete-branch --admin`, logged to `.claude/bypass-log.jsonl` (`pr-grind-admin-on-approver-gap` for an operator-confirmed merge; the `-solo-admin-auto` suffix for pr-grind's automatic path). That automatic path additionally requires pr-grind's Step-0 per-PR anti-self-bypass snapshot, so a grind that never ran Step 0 (e.g. an inline `--opus` run) escalates manually (operator-confirmed). The opt-in self-revokes if a second approval-capable human is ever added. Local tooling still preferred over remote CI for development workflow.
