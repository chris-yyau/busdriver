...
**Related opt-in — Codex `none`-nudge when Codex never auto-triggers (ADR 0013, issue #298):** `.claude/pr-grind-codex-expected.local` (gitignored). When present, pr-grind's COMPLETION first-engagement grace posts one `@codex review` **before** its existing bounded re-poll — so a repo that expects Codex to auto-review, whose Codex silently no-op'd on a fresh PR (quota exhaustion / dropped webhook), gets one nudge instead of merging without a Codex review. **Absent → today's behavior** (a `none` Codex is never nudged). Extends ADR 0005's `stale`-only retrigger to the `none` case via a thin opt-in policy wrapper `scripts/codex-nudge-if-expected.sh` that delegates to the same `codex-retrigger.sh` mechanism (shared one-shot marker → at most one `@codex review` per (PR,HEAD) across the `stale` AND `none` paths). **Bounded, never hangs:** reuses the existing grace re-poll; quota-dead / uninstalled / silent Codex all degrade to non-gating `none`. Merge authority (required checks + litmus) unchanged. Global kill switch `PR_GRIND_CODEX_RETRIGGER=0` disables it too. See `docs/adr/0013-codex-none-nudge-opt-in.md`.

**Related opt-in — advisory-bot stale-ack timeout downgrade (ADR 0012, issue #295):** `.claude/pr-grind-advisory-downgrade.local` (gitignored). When present, pr-grind — at `--max-wait` exhaustion, as the last step before BAIL — may downgrade a `stale` **advisory** reviewer ack (cursor/cubic/coderabbit/devin/codex, all treated uniformly) `stale → none` (non-gating) when that bot found 0 findings, has 0 unresolved threads on HEAD, is not in a blocking review state, AND required checks + litmus are green. This releases a green PR held hostage only because a bot reviewed an old SHA and never re-acked HEAD after a rebase — it NEVER touches merge authority (required checks + litmus still gate) and never downgrades a bot with live findings. The decision lives in `scripts/advisory-stale-downgrade.sh` (fail-CLOSED, single source of truth); each release logs an `advisory_stale_timeout_downgrade` event to `.claude/bypass-log.jsonl` and is surfaced in the completion message. The `pr-grind-clean.local` marker stays a bare PR number (never carries the list — `pre-merge-gate.sh` would treat non-digits as corruption). See `docs/adr/0012-advisory-bot-stale-timeout-downgrade.md`.
