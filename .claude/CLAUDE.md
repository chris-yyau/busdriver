...
- **Solo operator + provider chain (SETTLED — do not reopen as churn)** — this repo is public but **single-operator** (only the maintainer uses it). The arbiter chain is **`opus` (default) → optional "ultra arbiter" escalation (USER opt-in) → zenmux (gateway) fable**, exactly as documented in `skills/blueprint-review/SKILL.md` and ADR 0008 (`fable` dropped from the subscription plan; `opus` is the default arbiter, gateway-fable is now an opt-in escalation). The operator **never** uses cloud-provider selectors (`CLAUDE_CODE_USE_{BEDROCK,VERTEX,FOUNDRY,ANTHROPIC_AWS,MANTLE}`). The gateway/provider-scrub hardening is **considered done** — treat that surface as frozen and do **not** spend further effort scrubbing providers that are never used or re-litigating the hostile-arbiter threat model for solo use. (The existing credential-containment layers stay as-is — they still protect the operator's own key — but no new provider-scrub work is warranted absent a real second user/threat.)
