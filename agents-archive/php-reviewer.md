...
1. Establish review scope. Always pass the global `--no-pager` flag before the subcommand (`git --no-pager diff …`) — it disables the pager outright, so a malicious repo-local pager config cannot run. Do NOT rely on `-c core.pager=cat`: a per-command `pager.<cmd>` setting (e.g. `pager.diff`) overrides `core.pager` and could still execute.
   ...
   - If history is shallow or single-commit, try `git fetch --unshallow --filter=blob:none` first; if that fails, resolve a base ref (reuse `$BASE` from the PR-review step if set, else the branch's upstream: `BASE=${BASE:-$(git rev-parse --abbrev-ref --symbolic-full-name @{u} 2>/dev/null)}`) and the merge base (`MERGE_BASE=$(git merge-base HEAD "$BASE" 2>/dev/null || git merge-base HEAD "origin/$BASE" 2>/dev/null)`); only when `MERGE_BASE` is non-empty, run `git --no-pager diff "$MERGE_BASE"...HEAD -- '*.php'`. If the merge base is unresolved, report the shallow-history limitation and stop — do NOT fall back to `git show HEAD`, which reviews only the tip commit and misses earlier PR commits.
