...
1. Establish review scope:
   ...
   - If history is shallow or single-commit, try `git fetch --unshallow --filter=blob:none` first; if that fails, resolve the merge base explicitly (`MERGE_BASE=$(git merge-base HEAD "$BASE_BRANCH" 2>/dev/null || git merge-base HEAD "origin/$BASE_BRANCH" 2>/dev/null)` where `$BASE_BRANCH` is the base branch from step 1, trying both local and remote refs, quoted to prevent injection via ref names with shell metacharacters) and verify it is non-empty before running `git --no-pager diff "$MERGE_BASE"...HEAD -- '*.vue' '*.ts' '*.js'`; if the merge base is empty or unresolved, report the shallow-history limitation and stop — do NOT fall back to `git show HEAD`, which reviews only the tip commit and misses earlier PR commits.
   - Hardening: always pass the global `--no-pager` before the subcommand to disable pager-driven code execution via a repo-local `.git/config`. Do NOT rely on `-c core.pager=cat`: a per-command `pager.<cmd>` setting (e.g. `pager.diff`) overrides `core.pager` and could still execute.
