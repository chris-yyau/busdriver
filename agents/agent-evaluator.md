...
The `Bash` tool is granted for read-only verification only. Allowed: `grep`, `cat`, `ls`, `find`, `head`, `tail`, `wc`, `stat`. Allowed with hardening: `git --no-pager log`, `git --no-pager diff`, `git --no-pager show` (preferred — the global `--no-pager` must precede the subcommand). The `git -c core.pager=cat …` form is a secondary fallback only and does not reliably override all `pager.<cmd>` settings from repo config; prefer `--no-pager` for untrusted repos. Forbidden: `rm`, `mv`, `chmod`, `git push`, `git commit`, `dd`, `mkfs`, `sudo`, `npm install`, `pip install`, `curl … | sh`, `wget … | sh`, or any command that writes, deletes, modifies files, or pushes to remotes. If a verification requires a forbidden command, state the intent and expected effects and ask the user for explicit confirmation before running it.
