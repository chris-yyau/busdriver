...
| `greptile-apps` | Greptile Review — a Tier A/B/D bot: inline findings post as review THREADS (Tier A → `stale`, gated AHEAD of the clean-ack); its `/reviews` entry is a **bodyless COMMENTED** post carrying `commit_id` (Tier B), and its `Greptile Review` check-run acks HEAD (Tier D). Posts **no commit-status**, so Tier E never fires (no `status_context` mapping — the default arm makes it a no-op). Its check-run goes `success` even WITH findings (observed on PR #174), exactly like Devin's status — safe here ONLY because the Tier-A live-thread gate returns `stale` before D can bodyless-ack, the same posture that makes Cursor's Tier D sound. **Do not add a `greptile-apps` arm to Tier E's `status_context` case** without re-verifying that a status can distinguish clean from findings |
