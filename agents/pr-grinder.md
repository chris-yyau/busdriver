...
- **Tiers B (`/reviews`) and C (body-SHA):** these endpoints are PR-wide and still carry the pre-amend SHA, so `ack-ledger.sh`'s `acks_head()` HEAD-acks a candidate when git **proves** it has the same tree AND parents as the full HEAD OID. The candidate is sanitized (hex, 7–40 chars) before it reaches git.
