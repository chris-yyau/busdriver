...
| `greptile-apps` | Greptile Review — a Tier A/B/D bot: inline findings post as review THREADS (Tier A → `stale`, gated AHEAD of the clean-ack); its `/reviews` entry is a **bodyless COMMENTED** post carrying `commit_id` (Tier B), and its `Greptile Review` check-run acks HEAD (Tier D). Posts **no commit-status**, so Tier E never fires (no `status_context` mapping — the default arm makes it a no-op). Its check-run goes `success` even WITH findings (observed on PR #174), exactly like Devin's status — so Tier D is NOT clean-only. **Why the Tier-D ack is still safe against a "green check hides pending findings" fail-open:** a finding is delivered as a GitHub *review submission* — the Tier A inline threads and the Tier B `/reviews` `commit_id` entry are the **same review object**, published atomically (verified on PR #174: the finding comment's `pull_request_review_id` equals the `/reviews` id, same `commit_id`, same second). So the ledger can never observe a Tier B/A ack for a HEAD while that HEAD's findings threads are still invisible — they are one object, not a race. The only separately-published signal is the Tier D check-run, and reaching Tier D means Tier A/B already found NO review-with-findings for HEAD; the `Greptile Review` check's own completion (which pr-grind Step 1 `gh pr checks --watch` blocks on, + the 30s grace) is Greptile signaling its review is submitted. Same check-wait basis Devin's Tier-D ack relies on. **Do not add a `greptile-apps` arm to Tier E's `status_context` case** without re-verifying that a status can distinguish clean from findings |
