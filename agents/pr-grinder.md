...
```bash
ACKS="cubic-dev-ai=none,coderabbitai=none,greptile-apps=none"
ACK_TIERS="cubic-dev-ai=none,coderabbitai=none,greptile-apps=none"
CODEX_ACK=none                                              # Codex Tier-F ack; `none` = non-gating default
BOT_LEDGER="cubic-dev-ai=0/0:none,coderabbitai=0/0:none,greptile-apps=0/0:none,codescene-delta-analysis=0/0:none,chatgpt-codex-connector=0/0:none"
...
_cub=$(_at cubic-dev-ai); _cod=$(_at coderabbitai); _grp=$(_at greptile-apps)
ACKS="cubic-dev-ai=$(_ackpart "$_cub"),coderabbitai=$(_ackpart "$_cod"),greptile-apps=$(_ackpart "$_grp")"
ACK_TIERS="cubic-dev-ai=$(_tierpart "$_cub"),coderabbitai=$(_tierpart "$_cod"),greptile-apps=$(_tierpart "$_grp")"
...
**Your work:**
1. Step 1: wait for all GitHub-registered checks (`gh pr checks --watch`). This blocks on **check status**, not on review submissions — a bot can flip its check green seconds before posting. By the time Step 1 returns + the 30s grace period elapses, CodeRabbit has typically finished posting its review of `8947cdd`; Cubic often hasn't yet. The ledger (Step 6.5) is the authoritative ack signal — Step 1 only gives the bots a chance to start.
