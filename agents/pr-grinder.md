...
**Normalize before comparing.** Blame prints a full 40-char object ID; `commit=` values may be abbreviated. Resolve every stored value with `git rev-parse "<commit_value>^{commit}"` and compare full SHAs. Do NOT prefix-match: an abbreviation shared by an author commit and a grind commit would match the wrong one, and `rev-parse` is also what fails loudly on an ambiguous abbreviation.
