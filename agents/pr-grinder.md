...
`cursor` (Bugbot) and `devin-ai-integration` (Devin) were **dropped from this registry (ADR 0035)** — both are review-once-at-create bots that never re-review fix-round pushes; their findings were ~75-92% redundant with the remaining bots (60-day cross-repo audit), and the Case-4 devin whitelist (ADR 0027) that existed solely to un-strand Devin's clean reviews is removed. They still review PRs on the GitHub side; pr-grind just no longer gates on their acks or enumerates their findings.
...
_cur=$(_at cubic-dev-ai); _cod=$(_at coderabbitai); _grp=$(_at greptile-apps)
ACKS="cubic-dev-ai=$(_ackpart "$_cur"),coderabbitai=$(_ackpart "$_cod"),greptile-apps=$(_ackpart "$_grp")"
ACK_TIERS="cubic-dev-ai=$(_tierpart "$_cur"),coderabbitai=$(_tierpart "$_cod"),greptile-apps=$(_tierpart "$_grp")"
