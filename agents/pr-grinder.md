...
- `GRIND_SHAS` / `GRIND_SHAS_STATUS` / `GRIND_HEAD_SHA` — the **durable** provenance record (Rail A / ADR 0036): every commit on this branch that a grind wrote, including ones written by *earlier invocations* of pr-grind. `GRIND_SHAS` is a comma-separated list of full OIDs or the literal `none`; `GRIND_SHAS_STATUS` is `ok`; `GRIND_HEAD_SHA` is the full OID of the HEAD the set was scanned at — it binds the certified set to a specific snapshot so a concurrent invocation's newer commit can never be silently misread as author-written. All three travel together: `scripts/grind-pr-commits.sh --context` emits them as one snapshot, and `scripts/grind-set-member.sh` treats any partial combination as a contract violation. The dispatcher derives them with `scripts/grind-pr-commits.sh` before dispatching you and BAILs rather than sending an unverifiable set, so `unavailable` should never reach you. Where `PRIOR_ATTEMPTS` `commit=` is the *in-invocation* record — reset empty on every new invocation — this one survives across them. All three feed `GRIND_SET` in Step 3's proportionality gate. Absent on a pre-contract dispatcher; see the gate.
- `PRIOR_ATTEMPTS` — per-round bullet list. Each entry has the form `Round N (fix=<fix_round>/<MAX_FIX>, wait=<wait_round>/<MAX_WAIT>): commit=<sha or "none">; fixes=<one-line summary>; failures=<comma-separated failed-check-names or "none">; acks=<reviewer-ack-list>`. **`commit=` is the round's provenance record** — the SHA that round pushed, or `none` on a wait-round. It is the machine-checkable *in-invocation* attribution record (`fixes=` is free-form prose and `PRIOR_COMMIT_SHA` names only the latest push); Step 3's proportionality gate unions it with the durable `GRIND_SHAS` above. Attribute a finding by blaming its line (`git blame -w -M -C -L <line>,<line> --porcelain -- <path>`) and testing the resulting full SHA against these values after normalizing both — never by which files a commit touched. Absent on a pre-contract dispatcher — see the gate's fallback. The producer appends two further diagnostic fields, `scope-skipped=<n>; spawned=<n>` (Invariant 4's per-round contributions); they are optional suffixes for the operator reading a bail transcript, and you may ignore them. The parenthesized `fix=…/MAX_FIX, wait=…/MAX_WAIT` segment surfaces dispatcher budget pressure so you can triage knowing how close the loop is to bailing on either budget. **Anchor your parsing on `failures=` and `acks=` substrings, NOT on the `Round N:` prefix** — older worker contracts assumed the prefix shape; the new parenthetical breaks anchored parsers, but substring-anchored parsers are robust. Use `failures=` to detect a recurring flaky check across rounds (3+ rounds → bail; see Bail Triggers). `acks=` is preserved for diagnostics and human review of the loop transcript — there is no stuck-bot bail trigger; genuinely stuck bots fall out via the dispatcher's `--max-wait` iterations backstop (wait-rounds, where `RESULT_COMMIT_SHA=none`, count specifically against `--max-wait`).
- `PRIOR_REVIEWER_ACKS` — last round's ack ledger as a comma-separated list of `<login>=<value>` pairs, e.g. `cursor=b4451902,cubic-dev-ai=none,coderabbitai=none,greptile-apps=stale`. Values: short SHA (acked that commit), `none` (either never posted on this PR, OR the bot's only reviews are infra-error/rate-limit markers and it has never APPROVED, OR the bot acknowledged HEAD via a check-run with conclusion=skipped and non-actionable body — see Step 6.5's downgrade rule), or `stale` (posted a real review on an older commit and is expected to re-review HEAD). On round 1, `none` for every registered bot. See Step 2.5 (registry/concept) and Step 6.5 (compute) below.
...
```bash
ACKS="cursor=none,cubic-dev-ai=none,coderabbitai=none,greptile-apps=none"
ACK_TIERS="cursor=none,cubic-dev-ai=none,coderabbitai=none,greptile-apps=none"
CODEX_ACK=none                                              # Codex Tier-F ack; `none` = non-gating default
BOT_LEDGER="cursor=0/0:none,cubic-dev-ai=0/0:none,coderabbitai=0/0:none,greptile-apps=0/0:none,codescene-delta-analysis=0/0:none,chatgpt-codex-connector=0/0:none"
...
|---|---|
| `cursor` | Cursor Bugbot — acks HEAD via its `Cursor Bugbot` **success check-run** (Tier D, clean-only); inline findings post as review threads (Tier A). Re-registered by ADR 0041: on the Ultra plan with run-on-every-push enabled it re-reviews each fix-round push, which is the premise ADR 0035's drop rested on |
...
| `greptile-apps` | Greptile Review — gated via the **atomic review object only** (Tiers A/B/C), deliberately **off Tier D and E**. Inline findings post as review THREADS (Tier A → `stale`, gated AHEAD of any clean-ack); a clean review is a **bodyless COMMENTED** `/reviews` post carrying `commit_id` (Tier B), published atomically with any threads (verified on PR #174: the finding comment's `pull_request_review_id` equals the `/reviews` id, same `commit_id`, same second — so the ledger can never see a Tier B ack for a HEAD while that HEAD's threads are invisible); a summary issue-comment with a `Last reviewed commit/<sha>` link acks via Tier C and enumerates as an artifact (`n_total≥1`, normal gating). **Off Tier D** (the `ack-ledger.sh` login guard): its `Greptile Review` check goes `conclusion=success` even WITH open findings (observed on PR #174), so — unlike cursor/coderabbit's clean-only Tier-D check — a green check does NOT prove clean. A bodyless Tier-D ack would carry `n_total==0` and trip Invariant 3's ADR-0001 D/E exemption (auto-PASS), and the check-run is published separately from the review object with no read-after-write ordering, so it could be observed green while the findings review is not yet visible — a merge-past-findings **fail-open**. Being off Tier D, a check-run-ONLY clean run falls through to `none` (non-gating) instead of fabricating a clean ack. **Off Tier E** already (posts no commit-status; no `status_context` mapping). **Do not add a `greptile-apps` Tier-D login exemption or a Tier-E `status_context` arm** until a clean greptile run proves its check/status is clean-only |

`devin-ai-integration` (Devin) stays **dropped from this registry (ADR 0035)** — a review-once-at-create bot whose exclusive findings carried a 34% fix rate (60-day cross-repo audit), and the Case-4 devin whitelist (ADR 0027) that existed solely to un-strand its clean reviews is removed. It still reviews PRs on the GitHub side; pr-grind just no longer gates on its acks or enumerates its findings. `cursor` (Bugbot) was dropped by the same ADR and **re-added by ADR 0041** once the Ultra plan made it a review-every-push bot.

All four bots are gated through the same canonical algorithm in `scripts/ack-ledger.sh` (tiers A–E: inline threads, `/reviews` `commit_id`, issue-comment body SHA, check-runs, commit-statuses) — not bespoke per-bot body parsing. The REST API includes a `[bot]` suffix on logins (e.g. `cubic-dev-ai[bot]`) that the GraphQL/`gh pr view` paths strip; the Step 6.5 jq matches both forms.
...
**Codex (`chatgpt-codex-connector`) is gated, but via a SEPARATE field — `RESULT_CODEX_ACK`, not the four-bot `RESULT_REVIEWER_ACKS` registry above.** Codex reviews on pr-open and every push. When it has findings it posts inline review threads (Source 2) and/or a `/reviews` COMMENTED entry (Source 3) — both make `ack-ledger.sh` return `stale` (Codex is excluded from the Tier B `/reviews` clean-ack, and Tier A only clears a Codex thread that is **resolved AND non-outdated** AND anchored to the current HEAD — the thread's last comment must be resolver-authored and newer than `HEAD_PUSH_DATE` (which is why the fetch query carries `resolvedBy{login}` + `resolutionComments`); absent a push date this path fails CLOSED to `stale`. Live, outdated, or resolved-but-stale Codex threads stay `stale`), so the merge BLOCKS until the worker triages them and Codex either re-reviews clean (fresh 👍) or its threads are resolved on the current head. When it has NO findings it emits **one of two** clean signals — never a `/reviews` APPROVED entry, a check-run, or a commit-status. (1) A 👍 **reaction** on the PR body (per OpenAI's GitHub integration: "If Codex has suggestions, it will comment; otherwise it will react with 👍"); `scripts/ack-ledger.sh` **Tier F** reads it as a HEAD-ack when its `created_at` postdates `HEAD_PUSH_DATE` (the push event time; absent a push anchor it fails CLOSED to `stale`, #189) (verified on `chrisyau.me` PR #142 clean / #140 findings). (2) An **issue comment** whose body opens `Codex Review: Didn't find any major issues.` and carries a `**Reviewed commit:** <sha>` line; **Tier G** (#690) acks it when that SHA is HEAD — SHA-keyed, so no push anchor is consulted. Shape (2) is not a fallback for shape (1): observed on PR #687 and #688 it arrived with NO 👍 at all and the 👀 left in place, which read `stale` forever under Tier F alone. Either way the gate can WAIT for Codex without the deadlock a bare-👍-as-`none`/`stale` reading would cause on every clean terminal push. **Why a separate field and not `REGISTERED_ACK_BOTS`:** Codex's clean signal is Codex-specific (a reaction, or a templated comment), unlike the SHA-keyed structured acks of the registered bots; keeping it out of `RESULT_REVIEWER_ACKS` leaves the dispatcher's Invariant 3 intersection scoped to exactly those four. Codex is still enumerated for content in `RESULT_BOT_LEDGER` (Step 2.6), and a `stale` `RESULT_CODEX_ACK` blocks `clean` exactly like a stale registered bot.
...
**Proportionality gate — apply AFTER every finding this round is enumerated, before you apply any fix.** It needs the complete finding set, so it cannot run earlier.

The discriminator is **who wrote the code a finding sits on — a grind, or the author.** Provenance has two sources, and their union is the comparison set:

```
GRIND_SET = {resolved PRIOR_ATTEMPTS commit= values}   # this invocation
          ∪ {validated GRIND_SHAS entries}             # every invocation (durable)
```

Attribute a finding **per line, with `git blame`** — never by which files a commit touched. Note the shape of what blame actually returns:

```bash
# -w ignore whitespace, -M follow moves within a file, -C follow copies across files
git blame -w -M -C -L <line>,<line> --porcelain -- <path> | head -1
# → a porcelain HEADER LINE: "<sha> <orig-line> <final-line> [<group-size>]"
#   NOT a bare SHA. Take field 1.
```

**Do not hand-roll the comparison — call the helper.** The validation rules below are fail-CLOSED in one direction and fail-OPEN in the other, which is exactly the kind of asymmetry prose loses. `scripts/grind-set-member.sh` implements them and is covered by `tests/test-grind-set-member.sh`:

```bash
# Every <...> below is a LITERAL you substitute from your context block before
# running this. Shell state does not survive between Bash tool calls, so
# "$WORKTREE_DIR" would expand to empty and -C would fail.
...
# exit 2 (usage error) or ANY other status → BAIL `env` too, quoting stderr.
# Do NOT let an unexpected status fall through to the "uncertain means
# author-written" rule below: that rule is for genuine attribution ambiguity,
# and reusing it for a malformed invocation would silently disable the gate
...
- **The two sources have different failure rules, deliberately.** `PRIOR_ATTEMPTS` `commit=` keeps the inherited fail-OPEN behavior: `commit=none` is the wait-round sentinel and any value that does not resolve is dropped. `GRIND_SHAS` is **strict** — status must be `ok`, every token must be a full lowercase-hex OID that resolves to a commit, and **any violation BAILs `env`**. A dispatcher-certified set that does not validate is a broken contract, not a soft signal; carrying fail-OPEN across would let a corrupted durable set degrade silently back to the inert gate this exists to end.
- **Normalization is on both sides.** Blame prints a full OID while `commit=` values may be abbreviated, so each surviving value is resolved with `git rev-parse "<value>^{commit}"` before comparison. No prefix matching: an abbreviation shared by an author commit and a grind commit would match the wrong one.
- **An all-zero blamed SHA is author-written.** Blame emits `0000…0000` for a not-yet-committed line; it has no provenance, and `git rev-parse 0000…^{commit}` is fatal rather than merely empty, so it is skipped before normalization.
- **`GRIND_SHAS=none` with status `ok`** is a valid, empty durable set — not an error.
- **If the whole union ends up empty** there is no provenance to judge against: treat every finding as author-written and run the round normally.

The finding is loop output iff the blamed SHA is a member of `GRIND_SET`. **Never re-derive membership by matching the blamed commit's own message** — GitHub squash bodies concatenate every branch commit's message (verified on `924cbdea`), so `main` accumulates `Grind-PR:` lines and `-M -C` follows moves into merged history; a per-SHA message match would false-BAIL on author code. Set membership over the dispatcher's bounded range is what closes that; message matching does not. `git show --name-only` is the wrong tool here: it reports FILES, so a grind commit that touched the same file as an author-written finding would misattribute it. `-M -C` matter in the other direction: without them a grind commit that merely MOVED an untouched author line is credited with writing it.

**Blame is evidence, not proof — and that is acceptable here only because of where the error lands.** `-M`/`-C` use content-similarity thresholds and bounded copy sources, so a short or reformatted author-written line moved by a grind commit can still blame to that commit with no hint of uncertainty. The bounded consequence: this gate's ONLY action is a BAIL, which stops the round and hands every enumerated finding to the operator. It never resolves a thread, never dismisses a finding, never edits code. So a misattribution costs an unnecessary escalation — never a silently dropped defect. Do not add attribution machinery to chase the residual; if you find yourself wanting to, that is the signal to bail and let the operator judge.

Code the author pushed before the grind began — the fix, its tests, docs, cleanup — is theirs regardless of which commit came last, and findings on it are always fixed normally. If blame returns a SHA you cannot classify (e.g. the line came in with a merge commit), treat it as author-written.

**If `PRIOR_ATTEMPTS` entries carry no `commit=` field**, fall back to `GRIND_SHAS` — that is the whole point of the durable record, and it covers the common case of a fresh invocation on an already-ground PR. Only when BOTH are absent do you have no provenance record; then treat every finding as author-written and run the round normally. Do NOT substitute commit ordering, the PR's first commit, or `PRIOR_COMMIT_SHA` (which names only the LATEST push) — each of those misattributes author commits as loop output.

Establish the PR's intent first: the dispatcher passes `PR_NUMBER` but no PR body and no linked issue. Read `gh pr view "$PR_NUMBER" --json title,body,closingIssuesReferences` — the title often carries a requirement the body omits, so treat title and body together as the statement of intent; the body alone also misses issues linked through GitHub metadata. The reference list carries identifiers, not content, so read each one (`gh issue view <n> --repo <owner/repo>`, taking owner/repo from the reference so a cross-repository link resolves correctly). Zero linked issues is normal.

Then judge, **from the branch and not from any issue's open/closed state**, whether EVERY defect and acceptance criterion the PR set out to address is satisfied at the current HEAD. A PR may carry several — all must be closed out, since bailing while one is unresolved abandons it. Issue state proves nothing: `Fixes #n` stays open until merge, and a manually closed issue does not prove this branch fixed anything. **If any originating defect is unfixed, or you cannot determine what they were, this gate does not apply** — fix normally.

**A failing required check also switches the gate OFF, unconditionally.** Step 1 carries `$FAILED > 0` forward for resolution, and CI breakage is not a bot finding — it never appears in `BOT_REVIEWS`, so a gate that weighed only enumerated findings could bail past real breakage on author-written code. While any required check is red, fix it and run the round normally.

When all of them ARE fixed, sort the enumerated findings by that discriminator:

- **Any finding on author-written code** → the gate is OFF. Fix it and run the round normally; bailing would abandon it.
- **At least one finding, and every one of them on code a PRIOR GRIND ROUND introduced** → the loop is now reviewing its own output. **BAIL with `judgment`**, naming in `RESULT_BAIL_REASON` the originating defect, which rounds produced the code under review, and **every enumerated finding with its file:line** — the operator inherits them, so an attribution mistake costs a handoff rather than a lost defect. **Round labels exist only for the current invocation.** When attribution came from `GRIND_SHAS` with no matching `PRIOR_ATTEMPTS` entry, name the **SHAs** instead and say "prior invocation — round labels unavailable"; otherwise this clause is unsatisfiable in exactly the cross-invocation case Rail A exists to enable. The non-vacuity clause is load-bearing: with an EMPTY finding set "every finding" is vacuously true, and bailing there would divert a clean round from its normal completion path. No findings means proceed as usual, never bail.

**Cannot tell who wrote the code a finding sits on? Treat it as author-written and run the round normally.** Never bail on an uncertain classification. An empty `PRIOR_ATTEMPTS` no longer disables the gate: on round 1 of a *re-invocation* the durable `GRIND_SHAS` is typically non-empty, and that is precisely the case this gate was previously blind to (#620).

**Ledger on a proportionality bail.** The bail fires after enumeration, so the findings are known and must still be reported: count each in `n_actionable` and give the bot the disposition `skipped proportionality-bail` (the existing `skipped <reason>` shape — an enumerated finding the worker decided not to fix). It is NOT an out-of-scope dismissal, so append no `scope-skipped:` segment; that is what keeps the bail off the ≤3/≤5 budget. The all-`0/0:none` default belongs only to bails that fire BEFORE enumeration.

**This gate only ever BAILs — it never deletes code.** An earlier draft also let the worker *revert* later additions, on the theory that they were speculative. They are not reliably speculative: a prior round may have added that code to resolve a genuine correctness or security finding whose thread is now resolved, so reverting it would silently restore that bug with nobody left to re-report it. Distinguishing the two needs judgement the loop does not reliably have, so the decision goes to the operator instead.

Do NOT dismiss findings to reach a bail: a bail is not an out-of-scope classification and consumes none of the ≤3/≤5 discipline rails. **Honest limit** — the dispatcher routes on `RESULT_BAIL_CATEGORY` alone and never reads `RESULT_BAIL_REASON`, so nothing downstream can tell a proportionality bail from a design-question bail. This gate is enforced by you, not by the dispatcher; bailing to dodge a real finding is a contract violation no gate will catch. New attack surface found while fixing is a new issue, not more of this branch. Cheap signal you are here: `PRIOR_ATTEMPTS` shows ≥3 fix rounds and `BOT_REVIEWS` cite only hunks earlier rounds of THIS PR introduced.
...
# check-runs (Source 5) are fetched per-commit at HEAD only, so a bot's check-run on
...
_cur=$(_at cursor); _cub=$(_at cubic-dev-ai); _cod=$(_at coderabbitai); _grp=$(_at greptile-apps)
ACKS="cursor=$(_ackpart "$_cur"),cubic-dev-ai=$(_ackpart "$_cub"),coderabbitai=$(_ackpart "$_cod"),greptile-apps=$(_ackpart "$_grp")"
ACK_TIERS="cursor=$(_tierpart "$_cur"),cubic-dev-ai=$(_tierpart "$_cub"),coderabbitai=$(_tierpart "$_cod"),greptile-apps=$(_tierpart "$_grp")"
echo "Ack ledger: $ACKS"
echo "Ack tiers: $ACK_TIERS"
# Codex (chatgpt-codex-connector) — gated via Tier F (👍 reaction) and Tier G
# (clean-verdict comment naming HEAD, #690) + Tiers A/B (findings), but tracked in
# its OWN field, NOT folded into ACKS. ACKS is the contract for the three SHA-keyed
# structured bots; Codex's clean signal is Codex-specific — a reaction or a
# templated comment, not one of those structured acks — so keeping it separate
...
# --- Codex sole-stale-blocker auto-re-trigger (bounded N per HEAD) --------------
...
# then dead-end at --max-wait. Post `@codex review` so Codex re-reviews the
# current HEAD (→ fresh 👍/Tier-F ack, or new findings to triage next round),
# making the gate convergent. Call this on EVERY qualifying wait-round: the helper
# owns the budget (default 3 attempts per (PR,HEAD), paced by a cooldown — #673),
# so a nudge Codex drops or ignores is retried instead of being terminal for the PR.
...
| **Every originating defect already fixed AND all required checks green AND at least one finding, all of them on code a PRIOR GRIND ROUND introduced** (attributed via `GRIND_SET` — the union of `PRIOR_ATTEMPTS` `commit=` SHAs and the durable `GRIND_SHAS`) — the loop is reviewing its own output rather than closing the bug (Step 3 proportionality gate). Disqualified by: a failing required check, an empty finding set (never bail on a clean round), any finding on author-written code, any finding you cannot confidently classify, or an empty `GRIND_SET`. Absent `commit=` no longer disqualifies when `GRIND_SHAS` is populated — that is the #620 fix | **`judgment`** |
...
RESULT_CODEX_ACK: <single value from Step 6.5 — `<short-sha>` (Codex acked HEAD via Tier F 👍 newer than HEAD, Tier G clean-verdict comment naming HEAD, OR Tier A resolved current-head thread; live/unresolved findings return `stale`), `stale` (Codex still reviewing, OR has live/outdated findings to triage, OR hasn't re-acked HEAD → blocks `clean`), or `none` (Codex not on this PR → non-gating). Always present; early-bail paths emit `none`. Tracked separately from RESULT_REVIEWER_ACKS because Codex's clean signal is Codex-specific (a Tier F reaction or a Tier G templated comment), not one of the SHA-keyed structured acks — keeping it out of that string leaves Invariant 3's intersection scoped to the four registered bots. Additive/backward-compatible: a missing tag means the worker predates Codex gating; the dispatcher's own COMPLETION re-query is the authoritative gate regardless.>
...
PRIOR_REVIEWER_ACKS=cursor=stale,cubic-dev-ai=stale,coderabbitai=stale,greptile-apps=stale
PRIOR_ATTEMPTS:
  - Round 1 (fix=1/5, wait=0/8): commit=3f10ba92; fixes=mkdir -p ordering in run-review-loop.sh; failures=none; acks=cursor=stale,cubic-dev-ai=stale,coderabbitai=none,greptile-apps=none
  - Round 2 (fix=2/5, wait=0/8): commit=8947cdd; fixes=tilde expansion in target_dir parser; failures=none; acks=cursor=stale,cubic-dev-ai=stale,coderabbitai=stale,greptile-apps=stale
```
...
RESULT_BOT_LEDGER: cursor=0/1:approved,cubic-dev-ai=1/1:fixed SC2015 short-circuit,coderabbitai=0/1:approved,greptile-apps=0/0:none,codescene-delta-analysis=0/0:none,chatgpt-codex-connector=0/0:none
RESULT_ISSUES_SPAWNED: none
```

`needs_more` is correct — even with no remaining findings, all four registered bots still need to re-review `a1b2c3d`. Round 4 will wait in Step 1 for them to catch up; if no new findings emerge, Step 6 will be a no-op, Step 6.5 will compute against unchanged HEAD `a1b2c3d`, every bot will show `a1b2c3d` (acked HEAD), and that round can return `clean`.
