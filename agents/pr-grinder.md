...
The discriminator is **who wrote the code a finding sits on — this grind loop, or the author.** Each `PRIOR_ATTEMPTS` entry carries `commit=<sha|none>`, the SHA that round pushed; those SHAs, not commit ordering and not the free-form `fixes=` prose, are what identify loop output.

Attribute a finding **per line, with `git blame`** — never by which files a commit touched:

```bash
# -w ignore whitespace, -M follow moves within a file, -C follow copies across files
git blame -w -M -C -L <line>,<line> --porcelain -- <path> | head -1   # → full SHA
```

**Skip non-commit values, THEN normalize.** `commit=none` is the wait-round sentinel — that round pushed nothing, so it contributes no loop-authored code; drop it from the comparison set rather than resolving it (`git rev-parse "none^{commit}"` fails). Drop any other value that does not resolve, too. Blame prints a full 40-char object ID while `commit=` values may be abbreviated, so resolve each SURVIVING value with `git rev-parse "<commit_value>^{commit}"` and compare full SHAs. Do NOT prefix-match: an abbreviation shared by an author commit and a grind commit would match the wrong one, and `rev-parse` is also what fails loudly on an ambiguous abbreviation. **If the comparison set ends up empty** — every round was a wait-round, or no value resolved — there is no provenance to judge against: treat every finding as author-written and run the round normally.

The finding is loop output iff the blamed SHA matches one of the resolved `commit=` values. `git show --name-only` is the wrong tool here: it reports FILES, so a grind commit that touched the same file as an author-written finding would misattribute it. `-M -C` matter in the other direction: without them a grind commit that merely MOVED an untouched author line is credited with writing it.
...
**If `PRIOR_ATTEMPTS` entries carry no `commit=` field** (a dispatcher predating this contract), you have no provenance record: treat every finding as author-written and run the round normally. Do NOT substitute commit ordering, the PR's first commit, or `PRIOR_COMMIT_SHA` (which names only the LATEST push) — each of those misattributes author commits as loop output.
...
- **At least one finding, and every one of them on code a PRIOR GRIND ROUND introduced** → the loop is now reviewing its own output. **BAIL with `judgment`**, naming in `RESULT_BAIL_REASON` the originating defect, which rounds produced the code under review, and **every enumerated finding with its file:line** — the operator inherits them, so an attribution mistake costs a handoff rather than a lost defect. The non-vacuity clause is load-bearing: with an EMPTY finding set "every finding" is vacuously true, and bailing there would divert a clean round from its normal completion path. No findings means proceed as usual, never bail.

**Cannot tell who wrote the code a finding sits on? Treat it as author-written and run the round normally.** Never bail on an uncertain classification. On round 1 `PRIOR_ATTEMPTS` is empty, so no loop output exists yet and this gate can never fire.
