...
The discriminator is **who wrote the code a finding sits on — this grind loop, or the author.** `PRIOR_ATTEMPTS` is the record of what earlier ROUNDS of this loop changed; that, not commit ordering, is what identifies loop output. Code the author pushed before the grind began — the fix, its tests, docs, cleanup — is theirs regardless of which commit came last, and findings on it are always fixed normally.
...
| **Every originating defect already fixed AND at least one finding, all of them on code a PRIOR GRIND ROUND introduced** — the loop is reviewing its own output rather than closing the bug (Step 3 proportionality gate). Disqualified by: an empty finding set (never bail on a clean round), any finding on author-written code, or any finding you cannot confidently classify | **`judgment`** |
