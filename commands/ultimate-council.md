...
For the *initial* assignment above: never subshell, never `export`, never a `VAR=1 cmd` prefix (a trigger phrase alone sets no env var, and a one-command prefix would not persist to the later gate checks in the same block). Both witnesses route through the gateway (metered API billing, gateway creds from `BLUEPRINT_ARBITER_GATEWAY_*`); the Mythos Witness dispatches via `scripts/ultimate-dispatch.sh` (role `mythos-witness`, pinned `claude-fable-5`), fail-closed. Handing the already-set value to that child script IS a narrow, single-command `VAR=1 bash ultimate-dispatch.sh ...` prefix (see `skills/council/SKILL.md` Step 4.6) — that's a distinct, intentional use (scoping the non-exported var to one child process), not a violation of the rule above.
