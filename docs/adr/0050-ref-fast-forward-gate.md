...
| Downstream execution generally: the gate authorizes one ref move and cannot see what git runs afterwards. The case needing **no shell access at all** — `git merge` running `post-merge` from an in-tree `core.hooksPath`, where the landed content can BE the hook — is **closed by refusing the marker route whenever that path is in the tree**. The narrow "only if this merge changes it" check was tried and abandoned: resolving what git will execute means resolving repo-root paths, relative paths, symlinks, dangling symlinks, symlinks nested inside the hooks dir, and pathspec magic like `:(exclude)`, and each fix revealed the next hole | narrowed, not closed — the config-visible in-tree case refuses; a symlinked default `.git/hooks/post-merge`, a symlink inside an external hooksPath, an ancestor-symlink spelling, and an ambient `GIT_CONFIG_KEY_n` remain, and belong to the `reference-transaction` follow-up |
