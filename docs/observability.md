...
| `codex-droid-fallback` | litmus CLI (`resolve-cli.sh _execute_codex`) | Codex exhausted retries on transient errors (rate-limit / network / 5xx) or spent its full timeout; escalated to `droid exec` (default read-only mode) before falling back to builtin. Logged on every escalation regardless of droid outcome. `droid_outcome` records which one: `ok` (exit 0 with stdout — the review is used), `timeout` (exit 124, budget spent — the caller still sees 124 so it can split the diff), `no-output` (empty stdout inside budget, i.e. a silent refusal — clears the inherited timeout so it falls through to builtin as rc 3, #804), `failed` (non-zero exit with output). `droid_ok` is 1 only for `ok`. Classification keys on stdout alone, so an exit-0 diagnostic written to stderr is `no-output`, not a review. Disable with `LITMUS_CODEX_DROID_FALLBACK_DISABLED=1` |
