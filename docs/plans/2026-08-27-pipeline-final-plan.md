...
**2026-09-06 correction:** Busdriver's own `.claude/CLAUDE.md` is contributor guidance for
working on Busdriver. Its presence/size is not evidence that another project receives it by
...
At the source checkpoint, `hooks/hooks.json` registers SessionStart → `load-orchestrator.sh`, which
prefers the existing `skills/orchestrator/session-brief.md` and falls back to `SKILL.md`. That is
source-verified wiring, not a measurement of the operator's installed Claude, OMP or Cursor inputs.
...
role/model, instruction source, load trigger and evidence. Separate user-global/project instructions,
plugin startup context, on-demand skills/references and external-reviewer payloads. User-global and
consumer-project instructions have not been inspected in this audit; record missing visibility as
unknown, not absent. Use redacted traces or content identifiers/size metadata rather than publishing
private prompts or secrets. Where full prompt visibility is unavailable, label the partial evidence.
Only claim context savings or redundant work from observed delivery/reads and execution, not Markdown
length or repeated wording alone. This is targeted cleanup under existing items, not a new scheduler.
...
| 6 | **Extract orchestration mechanics, not routing policy:** move pr-grind's Dispatcher Loop into testable scripts first; later blueprint-review, council, and Litmus. Keep the existing stage/role mapping and `busdriver.json` resolution. Use the loading map to simplify descriptions and refine the EXISTING session brief, not add another one. SKILLs retain entry conditions, essential decisions, outputs and safety boundaries; executable mechanics become scripts/tests, and detailed recovery/history moves to references loaded when needed. Keep those references reachable at the point of use. #547/#662 remain separate reliability work. | hard; incremental | Semantic/output parity for shell state, cancellation, retries, partial output, resolved roles and gate decisions. No byte-for-byte requirement for implementations and no new free-form dispatch authority. Test required skill selection, reference/recovery reachability and unchanged gate behavior. Measure per-recipient context in consumer projects before/after; trimming Busdriver contributor docs alone is not evidence of plugin-wide savings. |
...
| 10 | **Reconcile host adoption with the deployed workflow:** inventory the existing Claude Code, OMP and Cursor-agent Busdriver adapters and their installed versions before continuing any Pi-replacement slice. Establish the loading map in a representative consumer worktree for each host and separately for external reviewers; keep Busdriver self-development measurements separate. Test their implementation of the SAME configured stages/routes and artifact contract; record host-specific enforcement gaps honestly. No new dispatcher or migration is implied. Keep Codex's configured review role and existing separate utilities. Reassess whether the historical `pi-goal-handover`/6-lite work is still needed. | reconciliation first; security changes need Chris | Per-host evidence for the same task/plan identity, resolved role/fallback, stale/missing review artifact, cancellation/resume and final authorization. Confirm which global/project instructions, startup brief, skill bodies and review payloads each recipient actually receives; unavailable captures remain explicit gaps. No claim of host parity from loading SKILL text alone. For any retained 6-lite path, preserve clean-worktree/operator-approved-base/untrusted-patch restrictions until a separately reviewed replacement exists; do not treat sandbox naming as proof of confinement. |
