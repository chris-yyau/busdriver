...
| 4 | Retire the file-level sync **mechanism** — `~/.claude/scripts/sync-upstream.sh` and its cache — but **keep `.upstream-sources.json`, frozen as the provenance inventory**, and keep BOTH `tests/test-upstream-manifest.sh` (schema guard) and `tests/test-provenance-guard.sh` (it SKIPs when the manifest is absent, `tests/test-provenance-guard.sh:175`, so removing the file would silently disable it). Evidence for retiring the copier is ADR 0048's own audit, not sync frequency: syncs did land (2026-04-15, 04-30, 05-29, 06-20, 07-07 — the earlier "one sync ever" claim was a grep artifact), but the synced files were never read or invoked (zero vault promotions in eight weeks), ADR 0048 then deleted most of them, and the cache dir is absent on the operator host; the last sync (2026-07-07) predates the trim. Generate `THIRD_PARTY_NOTICES` from files actually shipped, verified against each upstream's license. Upstream digest on demand only. Explicit ADR amending ADR 0048 D6 + ADR 0014. | hard | Disconfirming evidence = a `sync`-status file in the post-trim tree that was Read/invoked by a consumer after its last sync (`.claude/review-metrics.jsonl` / transcript grep); absent that, the copier stays retired. |
...
Design direction to validate next (three untested claims, one `claude -p` run each in the
scratch project — the prepared but NOT executed probe is below):
...
- (c) whether an exec-form hook's exit 2 / `{"decision":"block"}` still blocks, and how a
