...
| 1 | Fix `scripts/lib/resolve-cli.sh` grok preflight under the Bash tool (zsh: `BASH_SOURCE` empty → refuses → `council.researcher` falls to droid); droid re-auth or delete the fallback rung (closes #556). Isolate test writers from live logs — `~/.claude/homunculus/dispatch-log.jsonl` (~10.4k/15.6k rows in 60 d are tests) and `.claude/bypass-log.jsonl` (all event counts multiples of 39). `tests/test-pre-merge-gate.sh` already isolates `BUSDRIVER_STATE_DIR` (verified: 2810→2810 lines); candidates: `test-codex-premerge-warn.sh`, `test-gitcmd-detect.sh`, `test-gate-untrusted-cd.sh`, `test-relevant-check-status.sh`. The two logs have DISJOINT writer sets: `dispatch.sh:361` builds `LOG_DIR="$HOME/$STATE_DIR/homunculus"`, so overriding either `HOME` or `BUSDRIVER_STATE_DIR` redirects it, and every test invoking `skills/dispatch-cli/scripts/dispatch.sh` without one of those overrides appends to the LIVE dispatch log — Codex reproduced +3 rows from `tests/test-droid-escalation.sh` and +8 from `tests/test-cli-retry.sh` on a repo-root run; `test-dispatch-skipped-status.sh` and `test-pre-impl-deliberation-exempt.sh` set neither and are candidates pending the same per-test confirmation. Re-measure afterwards, and only once BOTH logs are quiet. | hard | `bash -c 'source scripts/lib/resolve-cli.sh; resolve_role_cli council.researcher'` → `grok`; under zsh → `droid`. Per-test `wc -l` diff on BOTH `.claude/bypass-log.jsonl` and `~/.claude/homunculus/dispatch-log.jsonl`, run over the whole shell suite (not just the named candidates); zero growth in both before re-measurement. |
