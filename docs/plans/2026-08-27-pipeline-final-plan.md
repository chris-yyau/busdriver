...
| 4 | Retire file-level `sync-upstream` (`~/.claude/scripts/sync-upstream.sh`, `.upstream-sources.json` as a sync manifest, `tests/test-upstream-manifest.sh`); keep `tests/test-provenance-guard.sh` (mentions sync-upstream in comments only). Generate `THIRD_PARTY_NOTICES` from files actually shipped, verified against each upstream's license, BEFORE deleting provenance machinery. Upstream digest on demand only. Explicit ADR amending ADR 0048 D6 + ADR 0014. | hard | `git log --grep=sync` shows one content sync (2026-04-07); disconfirm = a second. |
