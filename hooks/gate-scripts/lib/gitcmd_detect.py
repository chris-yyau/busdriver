        ...
        for op, seg in split_segments(chunk):
            ...
            if not raw and sub not in _ZERO_OLD_SUBS:
                ...
                if sub and (_may_be_substitution(sub) or _word_may_split(sub, sub)):
                    ...
                    # A read-safe subcommand writes no ref, so there is nothing for
                    # GIT_DIR/`-c`/a wrapper chdir to redirect -- refusing it was a
                    # pure over-block (same qualifier the merge arm carries, #812).
                    continue
