...
def _scan_segment(op, seg):
    ...
    # merge's PR number; main() aggregates seg_unsafe / is_merge / positional across segs.
    rec = {'op': op, 'cw': '', 'base': '', 'is_cd': False, 'is_merge': False,
           ...
           'had_reserved': False, 'seg_unsafe': False, 'positional': ''}
    if not seg.strip():
        return rec
    try:
    ...
    if cw == 'gh' and 'pr' in rest:
        ...
        if after and after[0] == 'merge':
            ...
            if positional and not re.match(r'^[0-9]+$', positional):
                rec['seg_unsafe'] = True
            rec['positional'] = positional
    return rec


def _evaluate_merge_first(recs, merge_index, target_dir, unsafe):
    ...
    # Returns (unsafe, target_dir). No-op (returns inputs) when there is no merge.
    if merge_index < 0:
        return unsafe, target_dir
...
def main():
    ...
    unsafe, target_dir = _evaluate_merge_first(recs, merge_index, target_dir, unsafe)
