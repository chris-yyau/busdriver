#!/bin/bash -p
# Native Git post-commit hook: consume merge-litmus pending claim after merge commit (#622).
...
gate_merge_consume_if_pending "$REPO_DIR" "$STATE_DIR" "post-commit-merge"
exit 0
