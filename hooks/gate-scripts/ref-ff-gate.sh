...
if [ -n "$ZERO_OLD_OPS" ]; then
    for _zo_pair in $ZERO_OLD_OPS; do
        _zo_name="${_zo_pair#*:}"
        if [ -z "$_zo_name" ]; then
            block_emit "BLOCKED: this would FORCE-UPDATE the protected branch (cannot be resolved statically; no old-oid precondition; issue #780)."
            exit 0
        fi
    done
fi
...
if [ -n "$ZERO_OLD_OPS" ]; then
    for _zo_pair in $ZERO_OLD_OPS; do
        ...
        _zo_name="${_zo_pair#*:}"
        if [ -z "$_zo_name" ]; then
            # Default-deref / unresolved ZERO-old still blocks as FORCE-UPDATE.
            block_emit "BLOCKED: this would FORCE-UPDATE the protected branch (cannot be resolved statically; no old-oid precondition; issue #780)."
            exit 0
        fi
