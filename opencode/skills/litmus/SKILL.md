...
**Example:**
```
You: "Review detected large diff (847 lines, 12 files). Auto-splitting..."
You: "Group 1: src/security/ (3 files, 230 lines)"
  → init → review → PASS → commit "feat: add path guard security module"
You: "Group 2: src/workers/ (2 files, 180 lines)"
  → init → review → PASS → commit "feat: add background worker"
You: "Group 3: src/utils/ (4 files, 290 lines)"
  → init → review → PASS → commit "feat: add utility modules"
You: "All groups committed successfully."
```

**State persists in:** `.opencode/litmus-state.md`

## Example: Silent Auto-Continue

```
You: "Running litmus review for commit 1 (database schema)..."
[Runs 3 iterations silently - user sees nothing]
You: "✅ Litmus review PASSED after 3 iterations. Committing..."
git commit -m "feat: Add notification database schema"
```

**Anti-pattern (DO NOT DO THIS):**
```
You: "Running review in background..."
You: "Still running... 20s"
You: "Still running... 40s"
You: "Review found 2 issues. Let me fix them..."
You: "Fixed! Re-running..."
You: "Still running... 20s"
[User falls asleep]
```
...
`terminal_status` field to `.opencode/litmus-state.md` immediately before
...
When `terminal_status` indicates the orchestrator could not converge — **not** when codex itself crashed — auto-dispatch `/codex:rescue` for a fresh diagnostic pass BEFORE surfacing to the user. This is distinct from the runtime `codex → droid → builtin` chain (which handles *transient* codex errors); this handles *logical* failure where review succeeded but you can't act on the findings.

| `terminal_status` | Auto-dispatch `/codex:rescue`? | Rationale |
|---|---|---|
| `stall` | **Yes — once** | Same blocking issues two iters in a row = you are stuck. Highest-ROI moment for a second opinion. |
...
```bash
git add -A                                                          # Stage changes
LITMUS="${BUSDRIVER_PLUGIN_ROOT}/skills/litmus/scripts"
bash $LITMUS/init-review-loop.sh 10                                  # Initialize
bash $LITMUS/run-review-loop.sh                                      # Review (auto-loops)
...
When the pre-PR gate blocks `gh pr create`, run the deep review: a **Codex xhigh deep multi-lens pass** (the lead reviewer, on the full `base...HEAD` diff) plus **ONE independent read-only Security/Bugs backstop subagent** (`pr-security-backstop`, dispatched via `task()`) for cross-model diversity on the two lenses that gate real harm — run opencode on a non-OpenAI family so the backstop diverges from the Codex lead. The gate is **machine-enforced**: the PR marker is refused unless BOTH a fresh PASS Codex-lead artifact AND a fresh PASS backstop verdict artifact exist, each bound to the current `base...HEAD` `diff_hash`. In PR mode a builtin/non-Codex lead is **fail-closed** (codex is pinned and `LITMUS_CODEX_DROID_FALLBACK_DISABLED=1` is set, so a failed Codex falls to builtin — which is rejected — never silently to droid).

**Full PR deep-review procedure** — Fast Path, pre-commit-coverage detection (Step 0.5), Codex deep pass, scope-drift detection, the single backstop dispatch + strict verdict artifact, gate decision, degraded states, and marker encoding: **Read `references/pr-review-mode.md`** when the pre-PR gate fires. It is not needed on the common commit path.

## Configuration (Environment Variables)

These govern both commit and PR review modes:
- `BUSDRIVER_REVIEW_CLI=auto` — choose review backend (auto/codex/agy/droid/builtin/none). Per-role routing: `.opencode/busdriver.json`
...
- `LITMUS_PR_FAST=1` — skip the independent Security/Bugs backstop (codex-only). Audited bypass: writes a distinct `PASS-FAST-<diff_hash>-<epoch>` marker logged as `pr-fast-bypass` in `.opencode/bypass-log.jsonl`; the gate honors it only via its explicit fast-bypass branch, never the normal backstop-gated path.
...
- `LITMUS_PR_BACKSTOP_MAX_DIFF` — max diff size (bytes) fed to the backstop prompt; an oversize diff fails closed (split the PR) rather than silently truncating to a PASS. Default `0` = no cap.
- `LITMUS_PR_BENCHMARK` — **planned follow-up, NOT yet wired** (deferred in ADR 0006; setting it currently has no effect). When built it would run Agy/Grok as non-gating observers logged to `.opencode/pr-review-benchmark.jsonl`. See `references/pr-review-mode.md` → "Benchmark Mode".
- `LITMUS_CODEX_DROID_FALLBACK_DISABLED=1` — opt out of the runtime droid escalation. By default (unset or `0`), when codex exhausts retries on transient errors (rate-limit, network, 5xx) — or hits a single full-duration timeout — the review escalates to `droid exec` (default read-only mode — Create/Edit blocked). On **transient-error** exhaustion a failed or unavailable droid then falls back to the builtin agent; a **timeout** that droid does not rescue instead preserves the timeout signal (exit 124) so the caller can split the diff — it does **not** fall to builtin. The legacy name `LITMUS_CODEX_DROID_FALLBACK=0` is also honored. Escalations are logged to `.opencode/bypass-log.jsonl` as `codex-droid-fallback` events. **Note:** this flag only governs the runtime fallback inside `_execute_codex`; install-time routes in `.opencode/busdriver.json` control which CLIs are tried for a role, but do not by themselves suppress this runtime escalation once codex is running. Use `LITMUS_CODEX_DROID_FALLBACK_DISABLED=1` (or `BUSDRIVER_REVIEW_CLI=codex`) when codex-only runtime behavior is required. (PR mode and blueprint-review both disable this fallback — see `LITMUS_CODEX_RETRIES`.)
- `LITMUS_CODEX_RETRIES=3` — maximum retry attempts before escalating to droid. Default: `3` (backoff 30, 60, 120 seconds ≈ 3.5 min, which clears OpenAI's per-minute window). **The most important review paths raise this to 5:** blueprint-review and litmus PR mode both export `LITMUS_CODEX_RETRIES=5` (backoff 30, 60, 120, 240, 480 ≈ 15.5 min, also outwaiting the per-5min rate-limit window) because those reviews are the gate of record and have no/limited droid net. Raise for longer patience; lower for faster bail (e.g., `export LITMUS_CODEX_RETRIES=2`). Retries fire on genuine network / rate-limit / 5xx transients (the original EAGAIN failure mode is now bypassed via `--prompt-file` — see Review Protocol above); a timeout is never retried (it escalates straight to droid).
...
1. Read the prompt path from `.opencode/builtin-review-prompt-path.local`
2. Read the review prompt from that path
3. Dispatch the `code-reviewer` agent via the task tool with the prompt as context. **The agent prompt MUST include:**
   - **Read-only mode:** "Do NOT modify any files. Report only. Do not use the Fix-First pass. Do not use the write or edit tools."
...
5. If no blocking issues: write the marker via `bash "${BUSDRIVER_PLUGIN_ROOT}/skills/litmus/scripts/write-review-marker.sh"` (NOT via write tool — the pre-implementation gate blocks writes to marker files)
...
When all conditions pass, litmus emits `⚡ Short-circuit PASS`, logs the event to `.opencode/review-metrics.jsonl` + `.opencode/bypass-log.jsonl` (as a distinct `short-circuit-pass` event, NOT a bypass), writes the commit marker, and exits 0 without dispatching Codex.
...
Every litmus review automatically logs its outcome to `.opencode/review-metrics.jsonl`:
...
**Environment variables:**
- `LITMUS_METRICS_FILE=path` — override metrics file location (default: `.opencode/review-metrics.jsonl`)
...
When the user wants to bypass litmus review (e.g., upstream-only syncs with no custom code), they create `.opencode/skip-litmus.local` manually in their terminal.
...
Both gates use the same skip file (`.opencode/skip-litmus.local`) and both enforce a 30-second timing heuristic against self-bypass.
