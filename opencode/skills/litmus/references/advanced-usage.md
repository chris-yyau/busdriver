...
bash ${BUSDRIVER_PLUGIN_ROOT}/skills/litmus/scripts/save_changelog.sh
```
...
# Check iteration
# Iteration is tracked in .opencode/litmus-state.md by run-review-loop.sh
...
outcome is appended to `.opencode/review-metrics.jsonl` with status, issues, severity
...
ITERATION=$(grep '^iteration:' .opencode/litmus-state.md 2>/dev/null | awk '{print $2}' || echo "1")
