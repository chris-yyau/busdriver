...
if [ -f ".opencode/pr-commits-prereviewed.local" ] && \
   grep -qxF "agents-only:${CURRENT_BRANCH}" ".opencode/pr-commits-prereviewed.local" 2>/dev/null; then
  echo "✅ All commits pre-commit reviewed — skipping codex CLI pass, agents-only mode."
  rm -f ".opencode/pr-commits-prereviewed.local"
...
After codex CLI passes, dispatch **6 parallel review agents** using the task tool. Each reviews the full `base..HEAD` diff from a different lens. Launch all 6 in a **single message** for concurrency.
...
```
This computes the diff hash and writes `.opencode/pr-review-passed.local`. Direct writes to marker files are blocked by the PreToolUse hook — only `run-review-loop.sh` can write them.
