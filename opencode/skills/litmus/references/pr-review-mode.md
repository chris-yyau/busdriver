...
- Exempt: `.opencode/`, `CLAUDE.md`, `docs/`, config files, and test files accompanying planned changes
...
Dispatch **exactly ONE** subagent via the task tool, `subagent_type="pr-security-backstop"` (NOT the old 6-agent fan-out). The agent is structurally read-only (`mode: subagent` with `permission: { edit: deny, bash: deny, webfetch: deny }`) — it cannot modify files or run commands. It runs on the session model; for cross-model value, run opencode on a non-OpenAI family so the backstop diverges from the Codex lead.
...
```bash
bash "${BUSDRIVER_PLUGIN_ROOT}/skills/litmus/scripts/run-review-loop.sh" --write-pr-marker
```
`--write-pr-marker` requires **BOTH** a fresh `status:PASS` `pr-codex-lead` artifact **AND** a fresh `status:PASS` `pr-backstop-verdict` artifact, both with `diff_hash` matching the current `base...HEAD`. A backstop PASS alone cannot satisfy the gate if the Codex lead was skipped, misordered, or FAILed — and vice versa. It writes `.opencode/pr-review-passed.local`. Direct writes to marker/artifact files are blocked by the PreToolUse hook — only the trusted writers can produce them.
...
```
After the gating Codex pass, each benchmark CLI runs via `execute_review` (isolated `BUSDRIVER_STATE_DIR`, per-`diff_hash` lock so the same diff is not re-dispatched) through its **own read-only extractor** (decoupled from the gating validator). Results append atomically to `.opencode/pr-review-benchmark.jsonl`:
```text
