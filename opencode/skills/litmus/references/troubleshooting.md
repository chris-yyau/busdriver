...
**Solution:** The skill passes the prompt via a temp file (`--prompt-file <path>`) rather than stdin. This sidesteps an EAGAIN race in the codex-companion's `fs.readFileSync(0, ...)` when fd 0 has `O_NONBLOCK` set (a stable condition under opencode's bash tool, not cleared by retries). The skill handles staged changes automatically — no manual diff piping is needed.
