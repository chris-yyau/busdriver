...
This skill is a **thin oracle dispatcher**. The actual round work runs in a fresh `pr-grinder` subagent on fixer, dispatched once per round. This:

- Cuts cost ~5× by running mechanical fix work on fixer
- Flattens conversation context — each round starts with O(1) tokens instead of O(N) accumulation across rounds
- Keeps oracle available for orchestration: triage of subagent results, bail handling, merge decisions, and skip-file protocol

**Override with `--opus`:** Run the loop inline in the parent oracle context (skips dispatch). Use when a PR has known nuance — multi-file architectural fixes, subtle review threads, etc.
...
START
  ...
  │     # across opencode bash tool calls (each tool call gets a fresh shell). The
  │     # dispatcher (you) MUST remember each flag's resolved value in
  ...
  │     # template values used throughout this SKILL.md — the dispatcher substitutes the
...
LOOP (terminates when fix_round >= MAX_FIX OR wait_round >= MAX_WAIT):
  ...
  │     task(subagent_type="pr-grinder", prompt=<context block>)
  ...
  │       BUSDRIVER_PLUGIN_ROOT="$BUSDRIVER_PLUGIN_ROOT" \
  │       PR_NUMBER="$PR_NUMBER" \
  ...
  │       bash "$BUSDRIVER_PLUGIN_ROOT/scripts/dispatcher-commit-block.sh"
  │
...
COMPLETION:
  ...
  ├── Write .opencode/pr-grind-clean.local at repo root
...
#   1. File:    .opencode/skip-baseref-check.local (touched in the user's terminal)
#   2. Env var: PR_GRIND_ALLOW_NON_MAIN_BASE=1 (exported in the PARENT shell
#              BEFORE launching opencode — inline `PR_GRIND_ALLOW_NON_MAIN_BASE=1
#              opencode` does NOT work because hooks fire before inline env applies,
...
if [ -f ".opencode/skip-baseref-check.local" ] || [ "${PR_GRIND_ALLOW_NON_MAIN_BASE:-0}" = "1" ]; then
  BASEREF_BYPASS=1
else
  BASEREF_BYPASS=0
fi
...
case "$BASE_BRANCH" in
  ...
  *)
    if [ "$BASEREF_BYPASS" != "1" ]; then
      ...
      echo "     - In your terminal: touch .opencode/skip-baseref-check.local"
      echo "     - Or in the PARENT shell BEFORE launching opencode: export PR_GRIND_ALLOW_NON_MAIN_BASE=1"
      echo "       (inline 'PR_GRIND_ALLOW_NON_MAIN_BASE=1 opencode' does NOT work — same rule as SKIP_LITMUS)"
      rm -f "$BASE_BRANCH_ERR"
      exit 1
    fi
...
if [ "$WT_EXIT" -ne 0 ]; then
  if printf '%s' "$WT_OUT" | grep -q 'already used by worktree at'; then
    ...
    # don't survive across bash tool calls; the printed marker is the
...
# merge block runs. The snapshot lives in the MAIN repo's .opencode/ (not the
# worktree's), because the operator's opt-in file is in the main repo and
# .opencode/*.local is gitignored / not copied into ephemeral worktrees.
...
if [ -n "$MAIN_REPO_ROOT_FOR_OPTIN" ] && [ -d "$MAIN_REPO_ROOT_FOR_OPTIN/.opencode" ]; then
  SOLO_OPTIN_FILE="$MAIN_REPO_ROOT_FOR_OPTIN/.opencode/pr-grind-auto-admin-solo.local"
  SOLO_OPTIN_SNAPSHOT="$MAIN_REPO_ROOT_FOR_OPTIN/.opencode/.pr-grind-solo-opt-in-snapshot-${PR_NUMBER}.local"
...
- `WORKTREE_DIR`, `BUSDRIVER_PLUGIN_ROOT`, `PR_NUMBER`, `RESULT_STATUS`, `RESULT_FIXES`.
...
When inline, the dispatcher executes the round triage/fix/stage body itself in the parent oracle context. Commit, litmus, push, and post-push ack synthesis still go through `scripts/dispatcher-commit-block.sh`, exactly like subagent mode.
...
RCS="${BUSDRIVER_PLUGIN_ROOT}/scripts/relevant-check-status.sh"
...
ACK_SCRIPT="${BUSDRIVER_PLUGIN_ROOT}/scripts/ack-ledger.sh"
...
**Acting on the result (instruction to the dispatcher, not shell control flow):** the `STALE_BOTS` variable lives only inside the Bash invocation that runs the snippet above — it does NOT persist into a subsequent inline-loop iteration. After the snippet runs, **the dispatcher reads the printed `Ack ledger:` and `STALE_BOTS:` lines from stdout** and decides:
...
- **STALE_BOTS non-empty** → round is in waiting-for-bots state; **skip Step 7 entirely** and re-dispatch Step 1 directly (analogous to the fixer subagent's `needs_more + RESULT_COMMIT_SHA=none + stale-acks` flow that the dispatcher's relaxed Invariant 1 permits). Increment the round counter as normal.
...
The `<PR_NUMBER>`, `<owner>`, `<repo>` placeholders below follow the same template-substitution convention as `<PR_NUMBER>` elsewhere in this Completion section — the dispatcher substitutes the literal owner / repo / PR-number values at run time before executing the bash.
...
# Fail-CLOSED: any error → conservative blocking "1 0 all 0".
REPO_DIR=$(git rev-parse --show-toplevel)
COUNTS=$(printf '%s\n' "$CHECKS_RAW" | bash "${BUSDRIVER_PLUGIN_ROOT}/scripts/relevant-check-status.sh" "$REPO_DIR" 2>/dev/null || printf '1 0 all 0\n')
read -r FAILED PENDING MODE KEPT <<<"$COUNTS"
...
**Write the pr-grind-clean marker (REQUIRED — pre-merge gate checks `.opencode/` at the REPO ROOT of the worktree the merge runs in). Run this as its own bash tool call:**
```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$REPO_ROOT/.opencode"
echo "<PR_NUMBER>" > "$REPO_ROOT/.opencode/pr-grind-clean.local"
rm -f "$REPO_ROOT/.opencode/pr-pending-grind.local"
```
...
Options:
  ...
  [admin]         gh pr merge <PR_NUMBER> --squash --delete-branch --admin
                    ...
                    # and writes NO entry to .opencode/bypass-log.jsonl. Same
...
# opencode bash tool calls (see EXTREMELY-IMPORTANT block near top). If CWD has
...
# Template-substituted by the dispatcher (you) at run time — the literal
...
# across opencode bash tool calls (each call gets a fresh shell), so the
...
# `.opencode/.pr-grind-solo-opt-in-snapshot-${PR_NUMBER}.local` exists AND its
...
# opt-in file and the snapshot live in the MAIN repo's .opencode/ (not the
...
if [ -n "$MAIN_REPO_ROOT_FOR_OPTIN" ]; then
  OPTIN_FILE="$MAIN_REPO_ROOT_FOR_OPTIN/.opencode/pr-grind-auto-admin-solo.local"
  ...
  OPTIN_SNAPSHOT="$MAIN_REPO_ROOT_FOR_OPTIN/.opencode/.pr-grind-solo-opt-in-snapshot-${PR_NUMBER}.local"
...
GAP_DECISION_JSON=$(bash "${BUSDRIVER_PLUGIN_ROOT}/scripts/approver-gap-detect.sh" 2>/dev/null \
...
- **`auto-admin-merge`** → log to `.opencode/bypass-log.jsonl` and run `gh pr merge <PR> --squash --delete-branch --admin`. Two triggers can reach this decision: `trigger=flag` (operator passed `--admin-on-approver-gap` per-invocation) or `trigger=solo-admin-auto` (operator placed `.opencode/pr-grind-auto-admin-solo.local` AND remains the sole human admin — see the "Solo-admin auto-detect" composer above). Both require the baseline gates: CI/bots clean (asserted by caller) AND author has admin/maintain AND `bypass-audit.yml` exists. Fail-CLOSED on any missing condition. The caller selects the bypass-log `event` value from `GAP_TRIGGER` so explicit vs. structural bypasses are distinguishable in forensics.

**Bypass-log format** (append-only JSONL; gitignored under `.opencode/`):
...
#   - PR / OWNER / REPO: re-template-substituted by the dispatcher (you)
...
jq -c -n \
  ...
  >> "$REPO_ROOT/.opencode/bypass-log.jsonl" || { echo "❌ failed to append bypass-log entry; aborting admin merge"; exit 1; }
...
.opencode/bypass-log.jsonl. For a logged merge, re-invoke with
...
# bash exports do not survive across opencode bash tool calls, so the
...
if [ "$NO_WORKTREE" = "1" ]; then
  ...
  mkdir -p "$ORIGINAL_REPO_ROOT/.opencode"
  cp .opencode/pr-grind-clean.local "$ORIGINAL_REPO_ROOT/.opencode/pr-grind-clean.local"
  rm -f "$ORIGINAL_REPO_ROOT/.opencode/pr-pending-grind.local"
  cd <original-worktree-path>
  git worktree remove "../pr-grind-<PR_NUMBER>" --force 2>/dev/null || true
fi
```
...
- Model: <fixer (default) | oracle (--oracle)>
...
| `--opus` | Run rounds inline in parent oracle context (no fixer dispatch) | Off (dispatches fixer subagent) |
...
| `--admin-on-approver-gap` | Opt-in auto-escalation when the approver gap is the sole remaining merge-gate blocker. Eligibility (ALL must hold): CI green, bots ack HEAD, all threads resolved, no failing required checks; author has `admin` or `maintain` repo permission; `.github/workflows/bypass-audit.yml` exists in the repo. With all gates green, the dispatcher runs `gh pr merge <PR> --squash --delete-branch --admin` and logs the event to `.opencode/bypass-log.jsonl` (`event: pr-grind-admin-on-approver-gap`). **Fail-CLOSED when no audit workflow exists** — the flag is ignored without a trail and the dispatcher surfaces the operator-decision message instead. Off by default. **Alternative — per-repo opt-in:** for repos where the operator is structurally the sole human with PR-approval capability (no other humans with write/maintain/admin could ever approve), drop `.opencode/pr-grind-auto-admin-solo.local` once (gitignored, same pattern as `skip-litmus.local`) and pr-grind treats the flag as implicit. The same eligibility gates apply, plus a live structural check that `HUMAN_ADMIN_COUNT==1` (counting humans with `permissions.push==true` — write/maintain/admin) and the author is that one approval-capable human. The opt-in self-revokes if a second approval-capable human appears — a contractor with write permission alone is enough to invalidate it. **Anti-self-bypass (snapshot-anchored, three conditions):** the opt-in file must be at least 30s old AT pr-grind INVOCATION START (Step 0), not at Completion. Step 0 snapshots the file's mtime to a per-PR snapshot at `.opencode/.pr-grind-solo-opt-in-snapshot-<PR>.local` (written 0600) only when the file is already ≥30s old; Completion auto-fires only when (1) the per-PR snapshot exists, (2) its recorded mtime equals the opt-in file's current mtime, AND (3) the snapshot file's own filesystem mtime is ≥30s after the opt-in file's mtime (defeats a same-NOW forge where an attacker creates both files in one action with identical mtimes). A mid-run touch (no snapshot) or mid-run replacement (mismatch) both invalidate the opt-in for the current run. The per-PR scoping prevents concurrent pr-grind runs on different PRs from racing on shared state. Snapshot and opt-in file both live in the MAIN repo's `.opencode/`, not the ephemeral worktree. The audit-log event is distinct: `pr-grind-admin-on-approver-gap-solo-admin-auto` with `trigger: "solo-admin-auto"` and `human_admin_count` recorded (variable name preserved for backward compat; semantic is now "humans with PR-approval capability"). | Off (surfaces decision message) |

## User-Created Skip File

When the user wants to bypass the pre-merge gate (e.g., pr-grind stuck in a loop, or PR ready-enough and the user accepts the risk), they create `.opencode/skip-pr-grind.local` manually in their terminal.

**Pre-merge specifics (different from other busdriver gates):**

- Skip file: `.opencode/skip-pr-grind.local`
- Trigger: `gh pr merge`
...
- **Gate:** Litmus runs inside the dispatcher-owned commit block before each fix commit; pre-merge gate fires on `gh pr merge` (skip: `.opencode/skip-pr-grind.local`)
- **Subagent:** `pr-grinder` (fixer) — receives one-round dispatch, returns RESULT_* tags. See `agents/pr-grinder.md`.
