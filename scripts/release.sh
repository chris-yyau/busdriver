...
# ── Maintain claude-release branch (strip opencode/ for Claude Code users) ──
# This block runs AFTER the tag is pushed. If it fails, the tag is already
# public but claude-release is stale — warn loudly but don't exit non-zero
# (the release itself succeeded; claude-release is a derived artifact).
echo "Maintaining claude-release branch..."
CLAUDE_RELEASE_BRANCH="claude-release"
ORIGINAL_BRANCH="$CURRENT_BRANCH"
# Disable set -e for this derived-branch block — failures here should warn,
# not abort the release (tag is already published at this point).
set +e
if git checkout -B "$CLAUDE_RELEASE_BRANCH" "v$VERSION" 2>/dev/null; then
    if [[ -d "opencode" ]]; then
        if ! git rm -r opencode/ || ! git commit -m "release: strip opencode port for Claude Code release

The opencode/ directory contains the opencode adapter plugin and skill
rewrites. Claude Code users don't need it — stripping it prevents token
waste from Claude exploring unused files during sessions.

Generated by scripts/release.sh during release of v$VERSION."; then
            echo "WARNING: Failed to strip opencode/ from claude-release. Tag v$VERSION is published." >&2
            echo "         claude-release NOT pushed — opencode/ may still be present." >&2
            echo "         Run manually: git checkout claude-release && git rm -r opencode/ && git commit --amend --no-edit && git push origin claude-release --force" >&2
            git checkout "$ORIGINAL_BRANCH" 2>/dev/null || git checkout main
            set -e
            echo "Released v$VERSION (claude-release update skipped)"
            exit 0
        fi
    fi
    if git push origin "$CLAUDE_RELEASE_BRANCH" --force; then
        echo "claude-release branch updated."
    else
        echo "WARNING: Failed to push claude-release branch. Tag v$VERSION is published." >&2
        echo "         Run manually: git push origin claude-release --force" >&2
    fi
else
    echo "WARNING: Failed to create claude-release branch. Tag v$VERSION is published." >&2
    echo "         Run manually: git checkout -B claude-release v$VERSION && git rm -r opencode/ && git commit && git push origin claude-release --force" >&2
fi
git checkout "$ORIGINAL_BRANCH" 2>/dev/null || git checkout main
set -e
