...
2. Pull stargazers (or forks for stronger intent) via `node tools/clis/github-prospects.js stargazers <owner/repo> --enrich --with-company --format csv` (CLI lives in the upstream [marketingskills repo](https://github.com/coreyhaines31/marketingskills) — clone it and run from its root; `tools/` is not vendored here)
