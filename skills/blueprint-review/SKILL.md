...
1. **Dispatch a fresh arbiter subagent** via the Agent tool:
   ...
   - `model`: **the arbiter pin tracks the calling session's model, clamped to an `opus` floor** — so the review gate is never *weaker* than the driver, and never silently weaker than `opus` (ADR 0025):
     ...
     - **Ultimate-arbiter escalation (opt-in) — force `fable` even when driving `opus`.** When the operator has opted in at the USER level (a top-level `.ultimate.surfaces.arbiter` in USER `~/.claude/busdriver.json`, or `BUSDRIVER_ULTIMATE=1` — never a repo-controlled config or reviewed content), pin `fable` regardless of the driver. Record `model_pin_status=ultimate_arbiter_fable`. Same fable pin chain as the driver-fable case.
