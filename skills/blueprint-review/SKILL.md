...
allow-rule is scoped to `$OUTPUT_FILE` and, under headless `--bare`, an Edit to any other path is
unapproved and denied. (`Edit` is an allowlist — one valid target; `Read` stays a blocklist — many
