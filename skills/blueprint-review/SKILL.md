...
> **Bash-tool timeout:** run this as a BLOCKING call (see the `<EXTREMELY-IMPORTANT>` block — while it blocks, the session *cannot* write `claude.json`, which is what stops a mid-flight verdict being built from partial artifacts) and **pass the tool's maximum `timeout` rather than computing one**. There is no correct number to compute: the phases are *(reviewers, `execute_review` default 1200s ‖ the k3 dispatch ‖ the oracle dispatch)* → the k3 residual reap (≤610s, its counter starts **after** the reviewer `wait`s) → an optional **sequential** droid rescue (one more `execute_review`) → the residual oracle wait, bounded by `ultra_oracle_timeout_cap + 90` from **dispatch** — **990s at the default cap, higher if `ultraOracle.timeoutCapSeconds` is raised** (ceiling 3600 ⇒ 3690s). Those are budgets, not bounds: `_portable_timeout` invokes `timeout`/`gtimeout` without `--kill-after`, the reviewer `wait`s are unbounded, and the perl fallback exits without reaping. In practice a round is ~8 minutes. If the call exceeds the foreground ceiling the harness backgrounds it and it runs to completion — treat that as a fallback, not a plan: the run is finished only when its completion signal arrives, and you must not read reviewer artifacts or write `claude.json` before then. To shorten a round, lower `ultraOracle.timeoutCapSeconds` or use the `.claude/skip-ultra-oracle.local` opt-out — **not** the witness budget.
