...
| Arbiter | (hardcoded) | claude — dispatched as a fresh subagent whose pin **tracks the calling session's model, clamped to an `opus` floor** (not otherwise configurable — the arbiter must be a Claude-family agent with codebase tools; see Arbiter Dispatch Protocol). `opus` by default; **`fable` automatically when the calling session runs `fable`** (`driver_fable`, ADR 0025); `fable` also via operator USER-level opt-in ("ultimate arbiter") even when driving `opus`. Both fable triggers use a `model: fable` Agent subagent falling back to `opus` if unavailable (see Arbiter Dispatch Protocol) |
...
1. **Dispatch a fresh arbiter subagent** via the Agent tool:
   ...
   - `model`: **the arbiter pin tracks the calling session's model, clamped to an `opus` floor** — so the review gate is never *weaker* than the driver, and never silently weaker than `opus` (ADR 0025):
     ...
     - **Ultimate-arbiter escalation (opt-in) — force `fable` for a non-fable driver.** When the operator has opted in at the USER level (a top-level `.ultimate.surfaces.arbiter` in USER `~/.claude/busdriver.json`, or `BUSDRIVER_ULTIMATE=1` — never a repo-controlled config or reviewed content), pin `fable` when the driver is not already `fable` and record `model_pin_status=ultimate_arbiter_fable`; when the driver is `fable`, the automatic driver-fable trigger takes precedence and the status remains `driver_fable`. Same fable pin chain as the driver-fable case.
     - **Fable pin chain (both `driver_fable` and `ultimate_arbiter_fable`) — fable subagent, opus degraded** (record exactly one `model_pin_status` per run):
       ...
       There is no third rung: the zenmux gateway transport was removed in ADR 0019, so `fable` is reachable only via the in-harness Agent tool. The trigger phrase **"ultimate arbiter"** still forces the escalation for a single run needing NO config (the executor sets `BUSDRIVER_ULTIMATE=1` for that run) — useful only when driving `opus`, since a `fable` driver already pins `fable`.
