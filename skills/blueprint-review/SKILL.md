...
- **Completed-but-hung recovery (#458, attach mode only).** oracle 0.16.0's browser engine can fail to detect that a *fast* gpt-5.5-pro response has finished (the streaming indicator vanishing is indistinguishable from "not started"), so the background run would otherwise wait out its whole `--timeout` and write no verdict while the completed answer sits in the live tab. In **attach mode** the adapter (`scripts/lib/ultra-oracle.sh`) now (1) **early-exits** the consult the moment its heartbeat shows the *confirmed* completed-but-hung signature (streamed, then stuck ≥2 ticks in the "no thinking status" waiting branch) — bounding the stall to seconds instead of the full `timeoutCapSeconds` — and (2) **salvages** the answer with `oracle session <id> --harvest` (re-reads the still-live tab), turning a dead hang into a captured advisory. Salvage fires **only on positive completion evidence** — the confirmed-hung early-exit, or an exit-0-but-empty capture (oracle concluded, extraction raced) — **never on a bare hard-cap timeout**, which could still be mid-stream and would risk harvesting a partial. The completed-but-hung early-exit requires **two agreeing heartbeat signals**: the streaming indicator gone for ≥2 waiting ticks *and* oracle's own "last change" reporting content stable for ≥`ULTRA_ORACLE_HUNG_STABLE` seconds. Because oracle itself cannot reliably distinguish "done" from "still working" (that ambiguity *is* the #458 bug), this remains a bounded **heuristic** — so a salvaged result is (best-effort) prepended with an explicit *"best-effort recovery — may be incomplete; auxiliary only"* caveat — and the advisory is non-gating regardless (the three reviewers + arbiter converge without it), so even in the rare case the label cannot be written the harvested note carries no gating weight. Non-attach modes are untouched (they keep the plain `_portable_timeout` and have no live tab to harvest). Tunable: `ULTRA_ORACLE_HUNG_GRACE` (default 45s minimum before the early-exit can fire), `ULTRA_ORACLE_HUNG_STABLE` (default 45s content-stability floor), `ULTRA_ORACLE_SALVAGE_CAP` (default 30s harvest budget).
