...
> **Bash-tool timeout:** run this as a BLOCKING call (see the `<EXTREMELY-IMPORTANT>` block — while it blocks, the session *cannot* write `claude.json`, which is what stops a mid-flight verdict being built from partial artifacts) and **pass the tool's maximum `timeout` rather than computing one**. There is no correct number to compute: the phases are **an attach-mode preflight** (oracle attach mode with a cold Chrome runs `scripts/ultra-oracle-attach-preflight.sh` SYNCHRONOUSLY, ~20-30s — `LAUNCH_WAIT_SECONDS=15` plus Chrome teardown — and it elapses **before** `ULTRA_ORACLE_DEADLINE` is anchored, so it is outside every budget below; zero when Chrome is warm or attach is off) → *(reviewers, `execute_review` default 1200s ‖ the witness dispatch ‖ the oracle dispatch)* → the witness residual reap (**≤1810s since the 2026-08-03 revision of ADR 0027**, up from ≤610s; its `_aud_grace` counter starts after the reviewer `wait`s but `AUDITOR_DEADLINE` is anchored at **dispatch** (#506), so it adds only ~610s past a worst-case reviewer wait rather than a full 1800) → an optional **sequential** droid rescue (one more `execute_review`) → the residual oracle wait, bounded by `ultra_oracle_timeout_cap + 90` from **dispatch** — **990s at the default cap, higher if `ultraOracle.timeoutCapSeconds` is raised** (ceiling 3600 ⇒ 3690s). Net serial worst case ≈ `attach_preflight + max(1200 + 610 + 1200, cap + 90)` — about 3010s at a 1800 cap, so size `BASH_MAX_TIMEOUT_MS` above that (3600000 fits with headroom). Those are budgets, not bounds: the reviewer `wait`s are unbounded. (`_portable_timeout` itself now passes `-k 5` to `timeout`/`gtimeout`, and its perl fallback puts the child in its own process group and reaps it, so a hung child is no longer the residual it once was.) In practice a round is ~8 minutes. If the call exceeds the foreground ceiling the harness backgrounds it and it runs to completion — treat that as a fallback, not a plan: the run is finished only when its completion signal arrives, and you must not read reviewer artifacts or write `claude.json` before then. To shorten a round, lower `ultraOracle.timeoutCapSeconds` or use the `.claude/skip-ultra-oracle.local` opt-out — **not** The witness budget.
