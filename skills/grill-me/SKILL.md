...
2. **Parse resolved decisions (only when `mode == resume_existing`).** For each line in the existing block matching `- **<decision name>** — chose <X>. Rationale: <Z>.`, treat that decision as already-resolved AND retain it. Bucket by Rationale prefix:
   ...
   Both lists MUST be carried verbatim into the new closing block — see the "Carry-forward rule" in the Closing Artifact section. Do NOT re-ask retained decisions (regardless of bucket) unless Step 2.5 or Step 4 explicitly transitions them. (Single canonical format — see Closing Artifact.) When `mode == fresh_grill`, SKIP this step entirely; both lists are empty by definition.

2.5. **Optional re-ask of self-decided picks (only when `mode == resume_existing` AND Step 2 populated `retained_self_decided` with ≥1 entry).** Fires ONLY when the user explicitly says any of: `revisit reversible`, `revisit self-decided`, `re-ask reversibles`, `re-ask self-decided` — at any point during the grill, right after Pre-Flight Step 2 completes, OR in response to Step 3's `complete` exit prompt. On trigger, list the self-decided picks by decision name (numbered) and prompt: "Which to revisit? `all` / `none` / comma-separated indices (e.g. `1,3`)". On user response, populate a `revisit_queue` ordered list:
...
- Honors the **opt-in re-ask keyword** (`revisit reversible` / `revisit self-decided` — see Pre-Flight Step 2.5) for the user to selectively re-grill prior `[self-decided]` reversible picks without nuking grilled decisions. Zero friction on normal resumes; fires only on explicit keyword.
