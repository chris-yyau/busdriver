...
> **Transport:** the image-generation steps below assume a host with a built-in image tool ("when image generation is available"). Claude Code has none — dispatch each image through `busdriver:imagegen` and continue with this skill's analysis and implementation phases as written.
