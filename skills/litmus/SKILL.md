...
**Iteration history:** After each FAIL, the issues found are saved to `.claude/litmus-iteration-history.local.jsonl`. On the next iteration, this history is injected into the prompt so the LLM knows what was already reported and can focus on verifying fixes. This file is per-loop-run — `init-review-loop.sh` clears it on init and a PASS clears it too, except that retiring a settled FAIL into the other mode archives it by rename instead (see the #847 note above).
