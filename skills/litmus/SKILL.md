...
5. If no blocking issues: write the marker via `bash "${CLAUDE_PLUGIN_ROOT}/skills/litmus/scripts/write-review-marker.sh" "<the prompt path from step 1>"` (NOT via Write tool — the pre-implementation gate blocks Write to marker files). The path argument is **required**: it says which handoff this review belongs to, so a delayed write cannot claim a handoff a later review armed (#790). The writer also exits non-zero without writing if another litmus run published a marker while this review was in flight — that marker stands; do not retry the review, re-run `/litmus` only if the commit is actually blocked. A third non-zero case is lock contention (another review in flight): nothing is consumed and the handoff stays armed, so if that run ends WITHOUT publishing a marker — findings, a timeout, an infra error — re-run the writer with the same prompt path once it finishes rather than re-reviewing (#794).
6. If blocking issues: report FAIL with issues, fix and re-run
7. Clean up: remove the temp prompt file **only**. Leave the handoff path file and its `builtin-review-marker-baseline.local` snapshot alone — `write-review-marker.sh` owns their lifecycle and consumes them itself. It deliberately preserves the pair in the two cases where deleting it would break a later run: on lock contention it leaves the handoff armed so the write can be retried, and when the handoff has been re-armed by a newer review it consumes nothing because the pair belongs to that run's agent. An unconditional delete here destroys exactly those cases.
