...
- `LITMUS_CODEX_RETRIES=5` — maximum retry attempts before escalating to droid. Default: `5`. Backoff sequence at the default is 30, 60, 120, 240, 480 seconds (~15.5 min total). Raise for longer patience before escalation; lower for faster bail (e.g., `export LITMUS_CODEX_RETRIES=2`).
