...
**2. Initialize:** `bash -p $LITMUS_SCRIPTS/init-review-loop.sh`
...
4. For each group:
   ...
   b. `bash -p $LITMUS_SCRIPTS/init-review-loop.sh`
...
Bash(command='bash -p "${CLAUDE_PLUGIN_ROOT}/skills/litmus/scripts/init-review-loop.sh" 10')
...
1. **Initialize the mode FIRST, and verify it.** `run-review-loop.sh` reads `review_mode`
   ...
   *run* call alone does nothing if a commit-mode state file exists — your "deep PR
   ...
   very issue (#363).
   ```bash
   LITMUS_MODE=pr bash -p "${CLAUDE_PLUGIN_ROOT}/skills/litmus/scripts/init-review-loop.sh" 10
...
```bash
LITMUS_MODE=pr bash -p "${CLAUDE_PLUGIN_ROOT}/skills/litmus/scripts/init-review-loop.sh" --force 10
```
...
LITMUS_SCRIPTS="${CLAUDE_PLUGIN_ROOT}/skills/litmus/scripts"
bash -p $LITMUS_SCRIPTS/init-review-loop.sh 10
...
bash -p $LITMUS/init-review-loop.sh 10                                  # Initialize
