...
4. Parse the agent's JSON output for blocking issues: any issue whose severity is not exactly `low` blocks (compare case-insensitively). Unrecognized or out-of-enum severities count as blocking. If parsing fails or the output is not a JSON array of issues, treat as FAIL.
