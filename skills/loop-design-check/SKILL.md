...
description: >-
  Design a goal-oriented agent loop, or review one for the ways loops go wrong
  — spinning, Goodhart-gaming the verifier, running a wrong answer to
  completion. Covers machine-decidable goals, loop-type choice, five failure
  modes, judge independence. Triggers: design/write/check a loop, loop review,
  runaway loop, decidable goal; 中文:写 loop、检查 loop、loop 体检。
metadata:
  origin: ECC
---
