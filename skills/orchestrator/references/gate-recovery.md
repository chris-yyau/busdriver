...
- NEVER verify the skip file via Bash (`test -f`, `ls`, `stat`, `cat`, `find`) before retrying. During the <30s window any gated call still destroys the design-review file, and for litmus/pr-grind Bash verification trips the same self-bypass detector. (Past that window the design-review skip is a lease evaluated AFTER tool-type discrimination, so a read-only call no longer spends a use — #519 / ADR 0031 — but verifying still tells you nothing useful.) In all cases: don't verify — just wait and retry.
