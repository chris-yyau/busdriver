...
| **UI/UX Design** | design, UI review, make it look better, styling | Dual-engine: `design-taste-frontend` (explore — landing/marketing/portfolio/showcase) → `impeccable:impeccable` (harden; owns dashboards/app UI solo) + `ui-ux-pro-max` + `busdriver:design-system` + `document-skills:frontend-design` gap-fill. Load `.impeccable.md` if present |
