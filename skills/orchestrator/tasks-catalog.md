...
| **Web Search (general)** | news, current events, broad lookups | `busdriver:tavily-cli` (search/extract/crawl/map/research suite — vendored Tavily CLI, free tier ~1k/mo); focused variant docs are archived — Read `skills-archive/tavily-search/SKILL.md` on demand (vault) |
| **Web Extract / Crawl / Map** | page extract, site crawl, list site URLs, discover pages | `busdriver:tavily-cli`; focused variant + API-guidance docs are archived — Read `skills-archive/tavily-extract/SKILL.md`, `skills-archive/tavily-crawl/SKILL.md`, `skills-archive/tavily-map/SKILL.md`, `skills-archive/tavily-research/SKILL.md`, `skills-archive/tavily-dynamic-search/SKILL.md`, `skills-archive/tavily-best-practices/SKILL.md` on demand (vault) |
| **Scrape / Crawl / Monitor a site** | scrape page, crawl docs, download site, watch for changes, JS-rendered/interactive pages | `busdriver:firecrawl`; focused variant docs are archived — Read `skills-archive/firecrawl-scrape/SKILL.md` (or -crawl, -map, -download, -interact, -monitor, -search, -agent, -parse) on demand (vault) |
| **Firecrawl app integration** | embed Firecrawl in product code, add FIRECRAWL_API_KEY, SDK setup, scrape/search/interact in an app | archived build guides — Read `skills-archive/firecrawl-build-onboarding/SKILL.md` (creds + SDK setup) then `skills-archive/firecrawl-build-scrape/SKILL.md` / `-build-search` / `-build-interact` on demand (vault) |
...
| **Agent Architecture** | agent loops, multi-agent DAGs, design/review a loop, runaway loop, decidable goal | `autonomous-loops` / `continuous-agent-loop` / `enterprise-agent-ops` (vault) / `agent-harness-construction` / `agentic-engineering` / `santa-method` / `autonomous-agent-harness`. Judgment layer (is the goal decidable, will it spin/cheat/run a wrong answer to done): `loop-design-check`. Agents: `harness-optimizer`, `loop-operator` |
...
| **Marketing Pack** | product marketing, positioning, messaging, CRO, copywriting, cold email, prospecting, SEO/AI-SEO/programmatic SEO, paid ads, ad creative, SMS, pricing, offers, referrals, churn prevention, onboarding, paywalls, popups, signup flows, launch, PR, revops, sales enablement, customer research, competitor profiling, lead magnets, marketing plan/psychology/ideas, GTM plan, fCMO plan, go-to-market plan, analytics, ASO, schema, site architecture, content strategy, co-marketing, community, directory submissions, emails, free tools, image/video production, social, copy editing, A/B testing | 45 vaulted skills adopted from coreyhaines31/marketingskills, each at `skills-archive/marketing-<name>/SKILL.md` (vault) — e.g. `marketing-cro`, `marketing-copywriting`, `marketing-cold-email`, `marketing-seo-audit`, `marketing-ab-testing`. Naming rule: upstream skill `<name>` lives at `marketing-<name>`; in-pack cross-references like "see cro" resolve to `marketing-cro` |
...
| **Web Research** | `busdriver:tavily-cli` (general/news + extract/crawl/map/research, free; per-command docs archived — Read from `skills-archive/` on demand (vault)), Exa MCP (`mcp__claude_ai_Exa__web_search_exa` — neural for code/papers/entities), `deep-research` (multi-source synthesis orchestrating Tavily CLI + Exa MCP; Firecrawl CLI for deep page extraction) |
