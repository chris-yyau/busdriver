...
- **Off-ramp updates:** When the user says "I get it" / "I know this now" / "stop explaining" for a specific domain, update the corresponding `user_*.md` memory file: either remove the domain from the entry's content or delete the file entirely if the entry is empty afterward. Update or remove the `MEMORY.md` pointer to match.
