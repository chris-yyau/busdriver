...
targets: impeccable:impeccable, document-skills:frontend-design
type: supplement
opt_in: true
...
> **Opt-in supplement.** Load alongside `impeccable:impeccable` or `document-skills:frontend-design` when the user wants to avoid generic AI-generated design patterns. Not loaded by default — invoke explicitly or when the user mentions "avoid AI slop", "make it unique", "don't make it generic".
>
> **Division of labor with `design-taste-frontend`** (personal skill, only if installed): for landing pages / portfolios / redesigns it already encodes a stricter superset of this blacklist (variance/motion/density dials + mechanical eyebrow/zigzag count-checks + GSAP skeletons) — prefer invoking it directly there. This file remains the fallback for dashboards / app UI / data tables, which `design-taste-frontend` explicitly excludes. If the skill is absent (machine without the install), this blacklist stands alone.
