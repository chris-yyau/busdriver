...
If ANY of criteria 1–4 fails → Outcome 3 (default executor). **When criteria 1–4 ALL hold AND every verifier is a deterministic runnable command (tests/lint/typecheck — not prose acceptance criteria) AND the tasks are mechanical (1–2 file changes each, no cross-cutting design decisions), prefer Outcome 1** — that shape is exactly what codex-goal's verifier-led loop handles well within its 8-iter cap, and the cost savings are real. For everything else — genuine doubt, marginal fit, non-deterministic verifiers, or design-flavored tasks — prefer Outcome 3, since a marginal-fit plan wastes the cost savings against that 8-iter cap. Expansion trigger: relax the size caps below only once >80% of handoffs land green within 4 of the 8 iterations (operator tracks this informally).
