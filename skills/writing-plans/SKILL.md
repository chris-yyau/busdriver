...
3. **Execute** — If Codex Handoff Eligibility selected Outcome 1: derive the literal spec path per the Slug derivation rule (record it in conversation context — e.g., `.claude/codex-goal-refactor-auth.json.local`), **emit the spec file at that literal path now** (deferred from the Eligibility step — see Outcome 1 for the JSON template), then INVOKE `busdriver:codex-goal-handover` with that literal path. **Cleanup on every exit path you control:** after handover returns (green, bailed, or max-iters) AND on any in-session error path before reaching that point (handover throws, a task blocker aborts Step 3), delete the spec by passing the literal path to `rm -f` (e.g., `rm -f .claude/codex-goal-refactor-auth.json.local`). Treat this as an always-runs finalizer for the paths you can act on — if you bail out of Step 3 for any reason you reach, do the rm before reporting to the user. This does NOT cover a hard session interruption (quota exhaustion, process kill), where no finalizer can run; the pre-flight orphaned-spec cleanup at the top of "Codex Handoff Eligibility" (specs older than 2 hours) is the backstop for that case, matching `busdriver:codex-goal-handover` Step 9. Do NOT rely on a `$SPEC_PATH` shell variable across Bash tool calls (each call spawns a fresh shell; the variable will be empty). Otherwise INVOKE `busdriver:subagent-driven-development` for independent tasks, or `busdriver:executing-plans` for dependent tasks requiring sequential execution with review checkpoints. (Outcome 2 already halted auto-execution before reaching this step.)
