...
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0"
eq   "$LAST_OUT" "cubic-dev-ai" "clean stale bot -> eligible"
...
run "cubic-dev-ai:0:0:APPROVED:old1:0:0"
eq   "$LAST_OUT" "cubic-dev-ai" "APPROVED terminal state -> eligible"

# 2. Solo opt-in absent -> nothing downgraded (fail-closed), no log.
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0" SOLO_OPTIN=0
empty  "$LAST_OUT" "no opt-in -> empty"
nofile "$LAST_LOG" "no opt-in -> no log event"

# 3-4. Global gates fail-closed.
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0" CI_GREEN=0
empty "$LAST_OUT" "CI not green -> empty"
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0" LITMUS_GREEN=0
...
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0" SERVER_NOW=
empty  "$LAST_OUT" "absent SERVER_NOW -> empty (fail-closed)"
nofile "$LAST_LOG" "absent SERVER_NOW -> no log event"
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0" SERVER_NOW=not-a-timestamp
...
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0" SERVER_NOW=2099-01-01T00:00:00Z
eq     "$LAST_OUT" "cubic-dev-ai" "far-future SERVER_NOW -> clamped, still eligible"
...
run "greptile-apps:0:3:COMMENTED:old1:0:0"
empty "$LAST_OUT" "actionable findings -> kept stale"
run "greptile-apps:0:0:CHANGES_REQUESTED:old1:0:0"
...
run "greptile-apps:0:0:PENDING:old1:0:0"
empty "$LAST_OUT" "PENDING last_state -> kept stale (not on allowlist)"
run "greptile-apps:0:0:SOME_FUTURE_STATE:old1:0:0"
...
run "greptile-apps:0:0:COMMENTED:old1:1:0"
empty "$LAST_OUT" "history ever had CHANGES_REQUESTED -> kept stale"
run "greptile-apps:0:0:APPROVED:old1:1:0"
...
run "greptile-apps:0:0:COMMENTED:old1::0"
...
LAST_OUT="$(env SOLO_OPTIN=1 CI_GREEN=1 LITMUS_GREEN=1 SERVER_NOW="$SRV" HEAD_SHA=head1 PR=1 REPO=o/r \
  ...
  CANDIDATES="cubic-dev-ai:0:0:COMMENTED:old1:0:0" bash "$SCRIPT" 2>/dev/null)"
...
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0,coderabbitai:1:0:COMMENTED:old2:0:0,greptile-apps:0:2:COMMENTED:old3:0:0"
eq   "$LAST_OUT" "cubic-dev-ai" "mixed set -> only clean bot eligible"
logn "$LAST_LOG" 1 "mixed set -> exactly one log event"

# 10. Two clean bots -> both eligible, two log events.
run "cubic-dev-ai:0:0:COMMENTED:old1:0:0,cubic-dev-ai:0:0:COMMENTED:old2:0:0"
eq   "$LAST_OUT" "cubic-dev-ai,cubic-dev-ai" "two clean bots -> both eligible"
