...
#   - an UPPER clamp (<= 600s)                       — can't stall arbitration for days
...
cn_norm() {  # <COUNCIL_AUDITOR_TIMEOUT value> -> normalized _AUD_TO
  ...
  code="$(awk '/_AUD_TO="\$\{COUNCIL_AUDITOR_TIMEOUT/{p=1} p{print} p&&/_AUD_TO" -gt 600/{exit}' "$COUNCIL")"
...
eq "$(bp_norm 0)"        300 "blueprint 0 (→ default)"
eq "$(bp_norm 9999999)"  600 "blueprint 9999999 (DoS bound)"
eq "$(bp_norm abc)"      300 "blueprint abc (non-numeric → default)"

eq "$(cn_norm 120)"      120 "council 120 (in-range)"
eq "$(cn_norm 00000600)" 600 "council 00000600 (zero-padded)"
eq "$(cn_norm 601)"      600 "council 601 (upper clamp)"
eq "$(cn_norm 0)"        120 "council 0 (→ default)"
eq "$(cn_norm 9999999)"  600 "council 9999999 (DoS bound)"

# Actual >64-bit overflow-sized digit strings: the result must stay bounded in
# 1..600 for BOTH normalizers (never abort, never wrap to an in-range garbage
# value that escapes the clamp).
for _nz in bp_norm cn_norm; do
  _ov="$("$_nz" 999999999999999999999)"
  if [[ "$_ov" -ge 1 && "$_ov" -le 600 ]]; then ok "$_nz overflow-sized input bounded → $_ov"
  else fail "$_nz overflow-sized input escaped 1..600 → $_ov"; fi
done
