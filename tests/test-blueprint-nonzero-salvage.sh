...
echo "── valid verdict + non-zero exit → salvaged ────────────────"
run_block "$TMP/valid-raw.txt" 7
[[ "$(j '.status')" == "FAIL" ]] \
  && ok  "complete verdict survives a non-zero exit (status FAIL, not ERROR)" \
  || bad "complete verdict survives a non-zero exit (status FAIL, not ERROR) — got '$(j '.status')'"
[[ "$(j '.reviewer_id')" == "droid" ]] \
...
[[ "$(j '.issues | length')" == "2" ]] && [[ "$(j '.issues[0].reviewer')" == "droid" ]] \
  && ok  "both issues survive and are retagged to the resolved reviewer" \
  || bad "both issues survive and are retagged (n=$(j '.issues | length') first=$(j '.issues[0].reviewer'))"
...
# derive_coverage reads runtime_escalated_from to mean "a droid rescue ran".
# Salvage is parse-only — no dispatch, no provider hop — so setting it would
# both misreport the slot and mislabel the #704 boundary.
[[ "$(j '.metadata.runtime_escalated_from // "unset"')" == "unset" ]] \
  && ok  "salvage does not claim a droid rescue (runtime_escalated_from stays unset)" \
  || bad "salvage does not claim a droid rescue — got '$(j '.metadata.runtime_escalated_from')'"
...
[[ "$(j '.status')" == "FAIL" && "$(j '.metadata.salvaged_exit_code')" == "3" ]] \
  && ok  "a BUILTIN_FALLBACK exit does not discard a complete verdict either" \
  || bad "a BUILTIN_FALLBACK exit does not discard a complete verdict — got status '$(j '.status')'"
