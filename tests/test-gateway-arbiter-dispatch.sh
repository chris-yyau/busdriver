...
run_script() {
  ...
  # standalone (HOME-isolated, no force) to cover the gate itself.
...
ULTRA_ISO_HOME="$TMPDIR_T/ultra-nooptin-home"; mkdir -p "$ULTRA_ISO_HOME"
rm -f "$STUB_LOG"
rc=0
env -i PATH="$PATH" HOME="$ULTRA_ISO_HOME" CLAUDE_BIN="$STUB_BIN" \
  BLUEPRINT_ARBITER_GATEWAY_BASE_URL=https://gateway.example/v1 \
  BLUEPRINT_ARBITER_GATEWAY_AUTH_TOKEN=tok-secret-123 \
  bash "$SCRIPT" "$PROMPT_FILE" "$OUTPUT_FILE" >/dev/null 2>&1 || rc=$?
check "full gateway creds but no opt-in skips with exit 3 (HOME-isolated)" 3 "$rc"
check "stub not invoked when creds set but opt-in absent" "absent" "$([[ -f "$STUB_LOG" ]] && echo present || echo absent)"
